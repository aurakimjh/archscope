...
| T-209 | P1 | [~] | **Skeleton landed; manual verification still required.** Added a "Browser support matrix (T-209)" section to both `docs/en/USER_GUIDE.md` and `docs/ko/USER_GUIDE.md` with: target-browser table (Chromium ≥120, WebKit/Safari ≥17.4, Gecko/Firefox ≥122), test setup steps (`pip install archscope` → `archscope serve --no-browser` → paste URL into each browser), a 25-row verification matrix covering static UI / file picker / drag-drop / multipart fallback / analyzer end-to-end / tab nav / WebSocket /ws/progress / cancel / ECharts / D3 SVG flame / Canvas flame auto-switch / D3 timeline+bar / image export PNG/JPEG/SVG / Save all charts / pprof / HTML+PPTX report / locale / theme / sidebar persist / shortcuts / Settings persist / Demo Data Center, all cells initialised to `☐ Not tested`. Per-browser caveat list (Chrome / Edge / Firefox / Safari) seeded for filling in. Pass/fail recording protocol documented (`✅ OK` / `⚠️ caveat` / `❌ #issue`). README cross-links the new section. Actual smoke runs across the four browsers fill the cells in a future PR. | T-208 | User feature request | Browser support matrix |
...
| T-310 | P1 | [ ] | 1 — simple parsers | Access log parser (`access_log_parser.py`) — nginx + apache common + custom regex format, malformed-line skip-with-diagnostics, `iter_text_lines` integration. | T-301 | `access_log_parser.go` + tests |
| T-311 | P1 | [ ] | 1 — simple parsers | Exception parser (`exception_parser.py`) — Java stack trace format + cause chain. | T-301 | `exception_parser.go` + tests |
| T-312 | P1 | [ ] | 1 — simple parsers | GC log parser (`gc_log_parser.py` + `gc_log_header.py`) — HotSpot unified GC log, JVM info card, pause/heap series. | T-301 | `gc_log_parser.go` + tests |
| T-313 | P1 | [ ] | 1 — simple parsers | OTel JSONL parser (`otel_parser.py`) — trace/span service paths. | T-301 | `otel_parser.go` + tests |
| T-314 | P1 | [ ] | 1 — simple parsers | JFR JSON parser (`jfr_parser.py`) + `jfr_recording.py` (`.jfr` → JSON via `jfr` CLI). Use `os/exec` for the JDK CLI bridge. | T-301 | JFR Go parsers + tests |
| T-315 | P1 | [ ] | 1 — simple parsers | Runtime stack parsers — `dotnet_parser`, `go_panic_parser`, `nodejs_stack_parser`, `python_traceback_parser`. Single Go file per language. | T-301 | Runtime parsers + tests |
| T-320 | P1 | [ ] | 2 — thread-dump registry | Port `parsers/thread_dump/registry.py` — plugin Protocol equivalent (Go interface) + 4 KB head sniffing + format override + `MixedFormatError`. Already partially in profiler-native; lift into shared engine module. | T-302 | Registry + interface |
| T-321 | P1 | [ ] | 2 — thread-dump registry | Java jstack plugin — full port of `java_jstack.py` including AOP cleanup (T-194), state inference (T-195), virtual-thread carrier pinning (T-227), SMR diagnostics (T-228 + T-234), native-method extraction (T-229), class histogram (T-230 + T-235), monitor semantics (T-231). | T-320 | java_jstack Go plugin |
| T-322 | P1 | [ ] | 2 — thread-dump registry | Java jcmd JSON plugin (`java_jcmd_json.py`). | T-320 | java_jcmd_json Go plugin |
| T-323 | P2 | [ ] | 2 — thread-dump registry | Go goroutine plugin (`go_goroutine.py`) + framework cleanup + state inference. | T-320 | go_goroutine Go plugin |
| T-324 | P2 | [ ] | 2 — thread-dump registry | Python plugins (`python_dump.py` py-spy / faulthandler). | T-320 | python_dump Go plugin |
| T-325 | P2 | [ ] | 2 — thread-dump registry | Node.js diagnostic-report plugin (`nodejs_report.py`). | T-320 | nodejs_report Go plugin |
| T-326 | P2 | [ ] | 2 — thread-dump registry | .NET clrstack plugin (`dotnet_clrstack.py`). | T-320 | dotnet_clrstack Go plugin |
| T-330 | P1 | [ ] | 3 — analyzers | Access log analyzer (`access_log_analyzer.py`) — 22-metric summary, percentile timeline, status breakdown, slow URL aggregation, error-over-time. | T-310 | `access_log_analyzer.go` + tests |
| T-331 | P1 | [ ] | 3 — analyzers | GC log analyzer (`gc_log_analyzer.py`) — pause / heap series, per-collector stats, JVM Info card. | T-312 | `gc_log_analyzer.go` + tests |
| T-332 | P1 | [ ] | 3 — analyzers | JFR analyzer (`jfr_analyzer.py`) + `native_memory_analyzer.py` — multi-event mode filter, wall-clock heatmap. | T-314 | JFR analyzer Go port |
| T-333 | P1 | [ ] | 3 — analyzers | Exception analyzer (`exception_analyzer.py`) + `runtime_analyzer.py` — type/root-cause/signature aggregation, runtime-specific findings. | T-311, T-315 | Exception + runtime analyzers |
| T-334 | P1 | [ ] | 3 — analyzers | OTel analyzer (`otel_analyzer.py`) — parent-span service paths, failure propagation, manifest expectation matching. | T-313 | `otel_analyzer.go` |
| T-335 | P1 | [ ] | 3 — analyzers | Single-dump thread analyzer (`thread_dump_analyzer.py`) — JVM-only state distributions, locks, signatures. | T-321 | `thread_dump_analyzer.go` |
| T-336 | P1 | [ ] | 3 — analyzers | Multi-dump correlation (`multi_thread_analyzer.py`) — `LONG_RUNNING_THREAD`, `PERSISTENT_BLOCKED_THREAD`, `LATENCY_SECTION_DETECTED`, `GROWING_LOCK_CONTENTION`, `THREAD_CONGESTION_DETECTED`, `EXTERNAL_RESOURCE_WAIT_HIGH`, `LIKELY_GC_PAUSE_DETECTED`, `VIRTUAL_THREAD_CARRIER_PINNING`, `SMR_UNRESOLVED_THREAD`, `INCOMPLETE_HISTOGRAM`. | T-320 + all plugins | `multi_thread_analyzer.go` |
| T-337 | P1 | [ ] | 3 — analyzers | Lock contention analyzer (`lock_contention_analyzer.py`) — owner/waiter graph + DFS deadlock detector. | T-336 | `lock_contention_analyzer.go` |
| T-338 | P1 | [ ] | 3 — analyzers | Thread-dump → flamegraph converter (`thread_dump_to_collapsed.py`) — already partially Go. | T-321 | `thread_dump_to_collapsed.go` |
| T-339 | P2 | [ ] | 3 — analyzers | Profile classification rules (`profile_classification.py`) — config-driven runtime classification. | T-301 | Classification table loader |
| T-340 | P1 | [ ] | 4 — exporters | JSON exporter (`json_exporter.py`) — round-trip-stable encoding. | T-301 | `json_exporter.go` |
| T-341 | P1 | [ ] | 4 — exporters | HTML exporter (`html_exporter.py`) — single-file portable HTML report with summary, findings, tables, chart data preview. | T-340 | `html_exporter.go` |
| T-342 | P2 | [ ] | 4 — exporters | PPTX exporter (`pptx_exporter.py`) — title, source metadata, summary metrics, findings slides. Use `unioffice` or hand-rolled OOXML zip. | T-340 | `pptx_exporter.go` |
| T-343 | P2 | [ ] | 4 — exporters | CSV exporter (`csv_exporter.py`) — summary/series flattening for spreadsheets. | T-340 | `csv_exporter.go` |
| T-344 | P1 | [ ] | 4 — exporters | Report diff (`report_diff.py`) — before/after numeric metrics + finding-count comparison. | T-340 | `report_diff.go` |
| T-350 | P0 | [ ] | 5 — web server | `apps/engine-native/cmd/archscope-engine/main.go` — `net/http` server replicating the 19 FastAPI routes (mux + middleware for CORS / static / progress WS). Use `nhooyr.io/websocket` for `/ws/progress`. | T-330–T-339, T-340–T-344 | Standalone Go HTTP server |
| T-351 | P1 | [ ] | 5 — web server | Static React mount via `embed.FS` — same wheel-packaging contract as Python (T-208). The Go build embeds the existing `apps/frontend/dist/` bundle directly; **the UI is NOT re-implemented in Go**, only the engine that serves it. Build helper (mirror of `scripts/build-archscope-wheel.sh`) runs `npm --prefix apps/frontend run build` then `go build -tags release` so the binary picks up the freshest assets. | T-350 | Embedded UI bundle |
| T-352 | P2 | [ ] | 5 — web server | Extract shared React components currently duplicated across `apps/frontend/src/components/charts/CanvasFlameGraph.tsx` and `apps/profiler-native/cmd/archscope-profiler-app/frontend/src/components/CanvasFlameGraph.tsx` into a workspace package (e.g. `packages/ui-shared/`) consumed by both apps. Today only `CanvasFlameGraph` lives in both; `DropZone` / `Sidebar` / `HorizontalBarChart` / `DiagnosticsPanel` are Wails-only; `MetricCard` / `ChartPanel` / `D3FlameGraph` are web-only — lift candidates as the duplication grows. Includes `npm workspaces` setup + Vite alias updates in both apps. | T-301 | Shared UI workspace package |
| T-360 | P1 | [ ] | 6 — CLI | Cobra root command for `archscope-engine` with the 14 subcommand groups (`access-log`, `gc-log`, `profiler`, `thread-dump`, `exception`, `nodejs`, `python-traceback`, `go-panic`, `dotnet`, `otel`, `jfr`, `report`, `demo-site`, `serve`). Each subcommand wraps the corresponding analyzer/exporter. | T-330–T-344, T-350 | Single-binary CLI |
| T-370 | P2 | [ ] | 7 — AI interpretation | Port `ai_interpretation/{evidence,validation,prompting,runtime,privacy,evaluation}.py` — Ollama-only client, evidence-ref grammar enforcement, prompt-injection guard, privacy redaction, golden-scenario evaluation harness. Optional feature; off by default. | T-336 | AI interpretation Go module |
| T-380 | P2 | [ ] | 8 — demo runner | `demo_site_runner.py` orchestration — read manifests, run per-scenario analysis, write JSON/HTML/PPTX bundles, normal-baseline comparison. | T-340–T-344, T-360 | `demo-site` CLI subcommand |
| T-390 | P1 | [ ] | 9 — CI parity gate | Extend `.github/workflows/profiler-native.yml`'s `python-go-parity` job to compare every analyzer surface (not just collapsed profiler) — golden fixtures per analyzer, JSON-canonicalised diff, drift fails CI. | T-330–T-344 | Full-engine parity gate |
| T-391 | P2 | [ ] | 9 — packaging | Single `archscope-engine` Go binary (replaces Python wheel for users who don't need the AI/Pretendard/full report stack). Wails v3 native profiler keeps its own binary; this binary is the CLI/server-only path. | T-350, T-360 | Binary distribution |
...
69. `T-330..T-339 + T-340..T-344 -> T-350 -> T-351`: web server lights up after every analyzer/exporter has a Go entry point + then the embedded UI mount.
69a. `T-351 -> T-352`: shared-UI-package extraction lands after the basic embed mount works, so the embed contract is locked before refactoring component homes.
70. `T-330..T-344 + T-350 -> T-360`: CLI subcommands wrap the same dispatchers the web server uses.
...
73. `T-330..T-344 -> T-390 -> T-391 -> T-392`: extend CI parity gate, then ship the single binary, then decide on the Python retirement window.
...
| Go HTTP / WebSocket libraries | T-350 | Default: `net/http` + `nhooyr.io/websocket` (Go 1.22 stdlib mux is enough; `nhooyr.io/websocket` over `gorilla/websocket` because it's context-aware and has a smaller surface). Cobra for the CLI (T-360). Revisit only if `net/http` mux falls short for path-parameter routing on the demo endpoints. |
| UI in the Go conversion scope | T-351 | **The UI is NOT re-written in Go.** Both React surfaces stay where they are: (a) `apps/frontend/` (42 TSX, the production web UI — Phase 1+2 closed under T-206..T-213, served as static bundle by the Go engine via `embed.FS`); (b) `apps/profiler-native/cmd/archscope-profiler-app/frontend/` (18 TSX, the Wails v3 webview UI — closed under T-240a..T-240f, packaged into the standalone profiler binary). Duplication between the two is tracked separately under T-352. Re-implementing the UI in a Go-native GUI toolkit (Fyne, Gio, etc.) was rejected during the Wails decision (2026-05-05) and is not revisited as part of the engine port. |
