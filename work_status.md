...
Last updated: 2026-05-06 (Phase 1 web pivot started: T-206 design note recorded in `docs/en|ko/ARCHITECTURE.md`, locking in FastAPI + uvicorn in-process runtime, HTTP + `/ws/progress` WebSocket transport, server-dialog-default file selection with multipart fallback, single-wheel packaging via `importlib.resources`, CSP/CORS posture for localhost, and the post-T-207 `apps/{frontend,profiler-native}` layout. T-001 (Electron + `child_process.execFile` IPC) is explicitly superseded. The Go/Native Profiler trail-end batch (T-240f / DebugLog UI / T-245 / T-246 / T-242 / T-247) shipped earlier in the day; that section has no open P1/P2 items left in the alpha scope. Final native binary 8.4 MB raw / 10 MB .app on darwin/arm64.)
