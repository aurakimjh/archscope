...
O2, and O4 are engine-scope and remain open as non-blocking. T-583 remains
`IN_PROGRESS` for the narrow X-RG1 re-review only.
**T-586 is complete**:
...
|---:|---|---|---|---|
| 1 | T-583 | P1 | `IN_PROGRESS` | X-RG1 review is `CONDITIONAL`; Codex B1/B2 and the Claude renderer remediation are both complete, so only the narrow X-RG1 re-review remains |
...
- Release baseline: `v0.3.5` is the latest stable GitHub release. The
  ...
  release candidate.
- Current execution focus: T-583 / X-RG1 review remediation after the
  2026-07-31 `CONDITIONAL` verdict. Codex B1/B2 remediation is complete:
  ...
  were taken in the same pass. Next: the narrow X-RG1 re-review. T-582 /
...
1. **IN_PROGRESS — T-583 / X-RG1 HTTP/profile/server-evidence correlation:**
   ...
   `npm run build`, `go build ./...`, and `go test ./...` all pass. Next:
   rerun the narrow X-RG1 re-review. O1, O2, and O4 remain engine-scope
...
| 7 | X-RG1 HTTP/profile/server-evidence correlation | Codex engine, Claude UI | IN PROGRESS — group review `CONDITIONAL`; Codex B1/B2 remediation complete, Claude unavailable-delta rendering remains before narrow re-review | Group PASS |
...
| T-583 | P1 | IN_PROGRESS | Implement and review X-RG1 HTTP correlation with Chrome/V8 CPU runs, Jennifer network-gap evidence, and access logs, including bounded alignment/confidence diagnostics and provenance-aware UI drilldown/overlay. | T-582 PASS (2026-07-30) | X-RG1 review `CONDITIONAL` 2026-07-31. Codex B1/B2 engine remediation and the Claude unavailable-delta renderer remediation are both complete with full Go/frontend verification; only the narrow re-review remains. Engine-scope O1/O2/O4 stay non-blocking; UI-scope O3/O5 are closed |
