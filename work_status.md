...
Last updated: 2026-05-06 (Go/Native Profiler trail-end batch landed: T-240f promoted to a real async + cancel path with `AnalyzeAsync` returning a taskId and Wails events `analyze:done` / `analyze:error` / `analyze:cancelled`; DebugLog (T-239g) now surfaces in the Wails service via `WriteDebugLog` and the Diagnostics tab "Save debug log" button; T-245 ships `docs/en|ko/PROFILER_NATIVE.md`; T-246 adds a global keyboard router (Cmd+O / Cmd+R / Esc / Cmd+, / Cmd+1..5); T-242 decided to keep the native profiler inside the monorepo for the alpha cycle; T-247 decided on a manual-download channel until Wails v3 stabilises and signing certs land. **The Go/Native Profiler Follow-up section now has no open P1/P2 items left in the alpha scope.** All 5 profiler input formats wired end-to-end; cross-engine Python ↔ Go parity blocks on CI. Final binary 8.4 MB raw / 10 MB .app on darwin/arm64.)
