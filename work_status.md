...
Last updated: 2026-05-06 (Wails v3 native profiler complete through T-239a/b/c/d/e/f/g, T-240a/b/c/d/e and the first slice of T-240f, plus T-241 golden parity tests + benchmarks, T-243 multi-platform packaging Taskfile + docs, T-244 GHA CI matrix. All 5 profiler input formats (collapsed / Jennifer CSV / FlameGraph SVG / async-profiler HTML / inline-SVG HTML) wired end-to-end through the Wails service and the native UI; cross-engine Python ↔ Go parity now blocks on CI. Final binary 8.4 MB raw / 10 MB .app on darwin/arm64.)
...
| T-244 | P1 | [x] | Added `.github/workflows/profiler-native.yml` with three jobs: (1) `go-test` matrix `[ubuntu-latest, macos-14, windows-latest]` running `go vet`, `go build`, `go test ./internal/... -race`, and a 1×iteration bench smoke; (2) `python-go-parity` matrix `[ubuntu-latest, macos-14]` building the Go CLI, installing the Python engine, running both pipelines on `examples/profiler/sample-wall.collapsed`, and asserting `type` / `summary.total_samples` / `summary.profile_kind` / `metadata.parser` / `series.top_stacks[0].{stack,samples}` agree (drift fails the build); (3) `package` matrix `[macos-14, macos-13, windows-latest, ubuntu-latest]` running `wails3 task package` per target and uploading `bin/*` as a versioned GHA artifact (`archscope-profiler-darwin-arm64` / `darwin-amd64` / `windows-amd64` / `linux-amd64`). Workflow is path-filtered to `apps/profiler-native/**`, `examples/profiler/**`, and the workflow file itself. | T-241, T-243 | User feature request | Native profiler CI gate |
