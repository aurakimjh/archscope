...
Last updated: 2026-05-06 (Wails v3 native profiler complete through T-239a/b/c/d/e/f/g, T-240a/b/c/d/e and the first slice of T-240f, plus T-241 golden parity tests + benchmarks, T-243 multi-platform packaging Taskfile + docs, T-244 GHA CI matrix. The CI parity job calls the Python CLI via the real subcommand `profiler analyze-collapsed --wall ... --wall-interval-ms ...` (not a hypothetical `analyze --collapsed` — the workflow file was corrected to match the actual Python CLI surface). All 5 profiler input formats (collapsed / Jennifer CSV / FlameGraph SVG / async-profiler HTML / inline-SVG HTML) wired end-to-end through the Wails service and the native UI; cross-engine Python ↔ Go parity now blocks on CI. Final binary 8.4 MB raw / 10 MB .app on darwin/arm64.)
