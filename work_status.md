...
Active cycle (2026-05-09, revised): the primary product line is now the unified Go/Wails implementation under `apps/engine-native`. The former Electron, Python/FastAPI browser, and standalone `apps/profiler-native` tracks are historical context only. Near-term work is release validation rather than another platform pivot: confirm the pushed GitHub Actions runs, cut `v0.3.0-rc1` once CI is green, smoke-test the macOS `.app` against the core sample families, and then address the remaining non-security Vite bundle-size warning through code splitting.

Next execution queue:

1. **Remote CI check** — verify the follow-up commit on `origin/main`; fix any remaining cross-platform workflow regressions before tagging.
2. **RC release** — tag and push `v0.3.0-rc1` after CI is green so `.github/workflows/release.yml` produces release artifacts.
3. **Manual desktop smoke** — launch the Wails app and run representative profiler, access log, GC, JFR, exception, thread-dump, export, and pprof flows.
4. **Bundle optimization** — split the Wails frontend's large production chunk if the release candidate confirms it is worth doing now.
5. **Signing/notarization** — populate Apple/Windows signing secrets when official distribution is required; current local builds fall back safely without them.
