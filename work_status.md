...
Last updated: 2026-05-07 (**TODO reorganization**: split remaining work into "Go track — active" vs "Python-coupled — deferred until T-392 retirement decision". **Active Go-only track**: T-360 (Cobra CLI), T-370 (AI interpretation, optional), T-380 (demo runner), T-391 (Wails desktop release flow — codesign / .dmg / Win/Linux builds / GitHub release). **Deferred until T-392** (Python wheel still ships, can't break it): T-352 (`apps/frontend/` cleanup — directly feeds Python wheel via `scripts/build-archscope-wheel.sh`) and T-209 (browser support matrix — verifies Python `archscope serve`, not the desktop path). When T-392 fires, T-352 and T-209 fold into the Python-removal sweep. Status `[⏸]` marks the deferred items. — older context retained below for reference. — **Tier-5 desktop-only pivot**: Go track shifted from "engine-native serves browser via HTTP" to "engine-native is a Go library; Wails desktop is the only user-facing binary". The agent-built `port/t-350-web-server` branch (`net/http` + `coder/websocket` server, 14 routes + WS) was deleted unmerged after the user reaffirmed the desktop POC's binary-size focus — HTTP code adds ~+1 MB dead weight to a binary the user never `serve`s. T-350/T-351 marked REJECTED; T-350'/T-351' added with Wails service bindings + desktop UI absorption scopes. T-352 expanded to cover `apps/frontend/` retirement decision. T-360 demoted P1→P2 (CLI now headless / CI / parity-gate only — `serve` subcommand removed). T-391 promoted P2→P1 as **primary release artifact** (single Wails desktop binary, target <12 MB raw). New ADQ row "Go binary deployment model" pins the desktop-only decision; "Go HTTP/WebSocket libraries" row marked OBSOLETE. Earlier today: full Tier-1 (5 P1 parsers), Tier-2 (registry + 6 thread-dump plugins), Tier-3 (9 analyzers including the 858-line multi-thread correlation), Tier-4 (5 exporters) all shipped via parallel agent dispatch — 30+ packages, 600+ tests, full module green under `-race -count=1`. Stack now: T-301 foundation through T-344 exporters all `[x]`, only T-360..T-392 + the new T-350'/T-351'/T-352 stream remain. — older context retained below for reference. — Added a new **Go Engine Full Conversion** track — T-301..T-392, scoping the entire Python engine surface (16 parsers + 7 thread-dump plugins + 18 analyzers + 6 exporters + AI interpretation + 19 web routes + 14 CLI command groups + demo runner) into a Tier-0..Tier-9 work plan. Module lands at `apps/engine-native/` so the existing `apps/profiler-native/` Wails track is unaffected. **UI is explicitly out of scope** — both React surfaces stay where they are (T-352 only extracts the components currently duplicated across `apps/frontend/` and `apps/profiler-native/.../frontend/`); the new Active Decision Queue row "UI in the Go conversion scope" pins this. Other new ADQ rows: module-location, retirement-window, HTTP/WebSocket libs. Earlier today: TDA-main hardening follow-up T-234/T-235/T-236 closed (SMR / histogram / payload pagination). Phase 1 web pivot + Phase 2 UI redesign closed (T-209 skeleton aside). Phase 1: T-206 design note + T-207 Electron removal + T-208 unified wheel + T-209 browser-matrix skeleton. Phase 2 gap analysis confirmed T-210/T-211/T-212/T-213 were already in place via prior sessions; the only outstanding bits filled in this slice were the Cmd/Ctrl+B sidebar toggle (T-211), a working search popover with Cmd/Ctrl+K and Enter-to-navigate over the 11 page labels (T-213), and ~350 lines of dead legacy CSS removed from `apps/frontend/src/styles/global.css` (T-212). Earlier in the day: `pip install archscope && archscope serve` is now the install path: distribution renamed `archscope-engine`→`archscope`, new `archscope` console script auto-opens the browser and inherits every analyzer subcommand from the engine CLI, React bundle ships as wheel package data resolved through `importlib.resources`, build helper at `scripts/build-archscope-wheel.sh`. `apps/desktop/` is gone; FastAPI gained `POST /api/files/select` + `WebSocket /ws/progress` + task-id keyed real `analyze/cancel`, all backed by a new `ProgressRegistry` symmetric with the Wails T-240f task registry. T-209 browser support matrix skeleton (25 verification rows × 4 browsers, all `☐ Not tested`) lives in `docs/{en,ko}/USER_GUIDE.md`; cells fill in with `✅ / ⚠️ / ❌` as someone runs the smoke against each browser. The Go/Native Profiler trail-end batch shipped earlier in the day; that section has no open P1/P2 items left in the alpha scope. Final native binary 8.4 MB raw / 10 MB .app on darwin/arm64.)
...
| T-209 | P3 | [⏸] | **DEFERRED until T-392 (2026-05-07 desktop pivot)** — this matrix verifies the Python `archscope serve` browser deployment, which is no longer the primary path post-pivot. Skeleton stays in `docs/{en,ko}/USER_GUIDE.md` so anyone running the Python wheel can fill cells; manual smoke runs across 4 browsers fold into the Python retirement sweep. | T-392 | Browser support matrix (deferred) |
...
Goal: port the **complete** Python analysis engine to Go so the entire ArchScope product (engine + web + CLI) can ship as a single Go binary, not just the profiler-first slice that already lives at `apps/profiler-native/`. Existing profiler-side Go code under `apps/profiler-native/internal/profiler/` is reused; the new code lands under a new top-level `apps/engine-native/` (Go module `github.com/aurakimjh/archscope/apps/engine-native`) so the profiler track is unaffected during migration. Python engine stays the source of truth until the Go version reaches CI parity (existing T-244 parity job extends as each surface ports).

**Scope inventory** (Python source tree, sized in Loc):
...
- **AI interpretation (6 modules)** — all remaining: `ai_interpretation/{evaluation, evidence, privacy, prompting, runtime, validation}.py`.
- **Web server** — FastAPI 19 routes (`/api/health`, `/api/settings` GET/PUT, `/api/upload`, `/api/analyzer/execute`, `/api/analyzer/cancel`, `/api/files/select`, `/ws/progress`, `/api/export/execute`, `/api/profiler/export-pprof`, `/api/demo/list`, `/api/demo/run`, `/api/files`, `/api/version`, plus the static React mount). All remaining; Go side will use `net/http` + `nhooyr.io/websocket` (or `gorilla/websocket`) and the same `ProgressRegistry` pattern already in `apps/profiler-native/cmd/archscope-profiler-app/profilerservice.go`.
- **CLI** — 14 typer command groups, 25 subcommands. *Already Go (profiler track):* `archscope-profiler` CLI with collapsed/jennifer/svg/html flags. *Remaining:* `access-log`, `jfr`, `gc-log`, `thread-dump`, `exception`, `nodejs`, `python-traceback`, `go-panic`, `dotnet`, `otel`, `report`, `demo-site`, `serve` and their subcommands. Go side will use `cobra` (or `spf13/cobra-cli`).
- **Demo runner** — `demo_site_runner.py` orchestration over `projects-assets/test-data/demo-site/` manifests. All remaining.
...
| T-352 | P3 | [⏸] | 5 — frontend cleanup | **DEFERRED until T-392 (2026-05-07)** — `apps/frontend/` is NOT just a Go-track artifact: `scripts/build-archscope-wheel.sh` copies `apps/frontend/dist/` into `engines/python/archscope_engine/web/static/` so the Python wheel can serve it via `archscope serve`. Deleting it now would break `pip install archscope && archscope serve`. The ADQ "Python engine retirement window" (T-392) commits to keeping Python alive until external users migrate. Until that decision lands, `apps/frontend/` stays frozen — no new features, but build still works. New analyzer pages go into the Wails frontend only (`apps/profiler-native/cmd/archscope-profiler-app/frontend/`). When T-392 fires the retirement, fold this task into the Python-removal sweep. | T-392 | Frontend unification (deferred) |
...
| T-370 | P2 | [ ] | 7 — AI interpretation | **GO TRACK — active.** Port `ai_interpretation/{evidence,validation,prompting,runtime,privacy,evaluation}.py` — Ollama-only client, evidence-ref grammar enforcement, prompt-injection guard, privacy redaction, golden-scenario evaluation harness. Optional feature; off by default. Pure Go module addition, no Python coupling. | T-336 | AI interpretation Go module |
...
| T-392 | P3 | [ ] | 9 — decision | Decide whether to retire the Python engine after T-390 reaches green and external users have migrated, or maintain both for an extended transition window. Decision-only. | T-390 | Migration end-state decision |
...
69a. `T-351' -> T-352`: frontend cleanup (delete `apps/frontend/` or lift shared components into `packages/ui-shared/`) lands after T-351' verifies feature parity in the Wails UI.
...
71. `T-336 -> T-370`: AI interpretation only after the multi-thread analyzer is stable (it carries the heaviest evidence shape).
72. `T-340..T-344 + T-360 -> T-380`: demo runner reuses the report family and the CLI wrappers.
73. `T-330..T-344 -> T-390 -> T-391 -> T-392`: extend CI parity gate, then ship the **single Wails desktop binary** (T-391 primary release artifact post-pivot), then decide on the Python retirement window. **2026-05-07 TODO reorg**: T-352 (frontend cleanup) and T-209 (browser matrix) were re-routed to depend on T-392 because both touch Python-coupled assets — they fold into the Python-removal sweep. Active Go-only stream: `T-360 (Cobra CLI) -> T-380 (demo runner)`, `T-336 -> T-370 (AI interpretation, optional)`, `T-391 (release flow finish)`.
...
| Go full-engine module location | T-301 | **Default: new `apps/engine-native/` Go module** (`github.com/aurakimjh/archscope/apps/engine-native`). Reuses `apps/profiler-native/internal/profiler/` types where the surfaces overlap (FlameNode, ParserDiagnostics, DebugLog, RedactText, AnalysisResult shell), but keeps the new analyzer/parser/exporter code in its own module so the profiler track's binary doesn't grow during conversion. Lift shared models into a `pkg/` directory once both apps reference them. |
| Python engine retirement window | T-392 | Default: keep both engines side-by-side until (a) every Go analyzer reaches CI parity (T-390 green), (b) external users have migrated for at least one minor release, and (c) `pip install archscope` keeps working as a thin wrapper that invokes the Go binary. Premature retirement is rejected — the AI interpretation track (T-370) and demo runner (T-380) lag the analyzer ports. |
...
| **Go binary deployment model** | T-350, T-391 | **Decided 2026-05-07: desktop-only.** The Go track ships exactly one user-facing binary — the Wails v3 desktop app (`archscope-profiler`). Engine-native is a pure Go library + thin CLI; profiler-native imports it via Wails service bindings (T-350'). HTTP server / `embed.FS` web bundle / browser deployment all rejected — they would add ~+1 MB dead weight to a binary the user never `serve`s. The browser route (Python `archscope serve` + FastAPI) stays available via the existing Python wheel for users who want it; the Go track does not duplicate it. Rationale: lightweight desktop is the explicit POC target the user has been tracking via binary size. |
| UI in the Go conversion scope | T-351 | **REVISED 2026-05-07 with desktop pivot.** The UI is NOT re-written in Go; the Wails v3 webview UI (`apps/profiler-native/cmd/archscope-profiler-app/frontend/`, 18 TSX today) is the **single user-facing UI** and absorbs the analyzer pages currently in `apps/frontend/` (42 TSX, the legacy web UI) under T-351'. After T-351'/T-352 land, `apps/frontend/` either becomes a workspace package of shared components (`packages/ui-shared/`, T-352) or is deleted. Re-implementing in a Go-native GUI toolkit (Fyne, Gio, etc.) stays rejected. |
