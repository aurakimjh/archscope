...
after a second independent `CONDITIONAL` verdict on 2026-07-29**. The re-review
...
updates the paired plan (R10), and persists aborted rows (R12). The schema-v3
...
the PowerShell harness itself still requires execution on Windows. T-582 stays
...
|---:|---|---|---|---|
| 1 | T-581 | P1 | `REVIEW` | Codex R1/R3–R8/R10/R12 and Claude R8/R9/R11 follow-up implemented; generate archived Windows schema-v3 evidence, then obtain independent `PASS` |
...
- Current execution focus: T-581 / H-RG4 Windows live-capture UI and E2E is in
  `REVIEW` after the 2026-07-29 independent re-review returned a second
  ...
  work is a fresh inspectable Windows schema-v3
  artifact from the expanded harness and independent
  re-review. Local full validation and Windows cross-compilation pass; actual
  PowerShell harness execution remains part of the Windows artifact step.
...
1. **REVIEW — T-581 / H-RG4 `CONDITIONAL`:**
   - **Codex backend — implemented:** R1 transaction-derived finalized
     ...
     analysis parity.
   - **Windows integration — next:** run the schema-v3 harness from a
...
| 5 | H-RG4 live UI and Windows E2E | Claude UI, Codex integration | REVIEW — Codex follow-up and Claude R8/R9/R11 handoff implemented; Windows artifact pending after second `CONDITIONAL` | Group PASS |
...
2026-07-28, and T-581/H-RG4 remains in `REVIEW` after a second independent
...
| T-581 | P1 | REVIEW | Implement and review H-RG4 Windows live-capture UI and E2E. Claude: capture controls, CA lifecycle UX, process tree, stable live rows, recovery/backpressure/coverage/fidelity states. Codex: frozen bindings, acceptance fixtures, Windows E2E and packaging support. Carry the H-SEC2 binding conditions: gate unknown-attribution retention behind an explicit metadata-only opt-in (SEC-17) before exposing stored transactions, and require the SEC-10 dump-exclusion preflight before enabling body capture. | T-580 PASS | The 2026-07-29 second `CONDITIONAL` follow-up is implemented for Codex-owned R1/R3–R8/R10/R12: finalized live provenance/fidelity and redaction are truthful; pinning/tunnel failures retain honest mode/fidelity/coverage/process; resolver work is connection-scoped and confirms start time; renderer behavior is a generated Wails contract bound to the fixture; paired plans are current; aborted rows persist. The expanded schema-v3 harness covers supported clients, h2-only, pinning, long-session, WebView re-entry, and recovery. The Claude-owned R8/R9/R11 handoff is also implemented: the renderer derives its row cap, resync, re-entry, and finalized handoff from `LiveCaptureContract` and discloses a contract it cannot honour, `isDecodedLiveFidelity` gates the live fidelity emphasis through `liveFidelityTone`, and transaction/session/CA/attribution states use closed EN/KO label maps. Full local and Windows cross-build validation pass. Remaining: execute and archive a fresh Windows artifact for R2, and obtain independent `PASS`. T-582 stays blocked. |
