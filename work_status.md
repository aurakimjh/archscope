...
|---|---|---|---|---|---|---|
| T-301 | P0 | [ ] | 0 — foundation | Bootstrap `apps/engine-native/` Go module + `internal/{models,common,diagnostics}` packages. Port `AnalysisResult` envelope (extend the profiler track's struct so the same shape holds Access Log / GC / Thread Dump etc.), `ParserDiagnostics` builder, `DebugLog`/`RedactText` (already Go — re-export from a shared internal package), `statistics.py` percentile/histogram helpers, `file_utils.py` encoding-safe text iteration with cp949/utf-16/utf-8-sig fallbacks. Add `go test ./...` to the existing GHA matrix. | T-244 (parity gate) | Foundation Go module |
