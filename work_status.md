...
Last updated: 2026-05-05 (Wails v3 native profiler now covers T-239a/b/c/d/e/f and T-240a/b/c/d/e and the first slice of T-240f, plus drag-drop + Recent files + collapsible sidebar + Settings page + Compare/Diff page + pprof export. All 5 profiler input formats (collapsed / Jennifer CSV / FlameGraph SVG / async-profiler HTML / inline-SVG HTML) wired end-to-end through the Wails service and the native UI. Final binary 8.4 MB raw / 10 MB .app on darwin/arm64. Reminder: `wails3 init` baked "probe" into `build/darwin/Info.plist` / `Info.dev.plist` — both corrected to ArchScope identifiers; changes to `build/config.yml` do not auto-propagate, run `wails3 task common:update:build-assets` if the `info` block is touched again.)
...
| T-239g | P2 | [ ] | Parser diagnostics + DebugLog parity for SVG/HTML/Jennifer in Go — match the Python `archscope-debug/` portable redacted log shape (line context, failed-pattern name, field shapes, redaction). | T-239b, T-239c | User feature request | Go portable parser debug logs |
...
| T-240f | P2 | [~] | First slice landed: 5-tab content layout (Summary / Flamegraph / Charts / Drill-down / Diagnostics) with auto-disable + Diagnostics red badge; spinner + locked options during `Analyze`; option strip auto-collapses after a successful run with an Edit / Hide options toggle. Recovered hidden information: Top child frames table, Timeline scope card (mode / base method / match mode / view mode / base samples / base ratio / warnings), Top stacks expanded 5 → 15 with internal scrolling, full Diagnostics tab (counts + skipped-reason chips + severity-coded per-sample lists with raw line preview). Cancel button is UI-only — actual cancel path waits on an `AnalyzeAsync` service method. | T-240a | User feature request | Native UX feedback first slice |
| T-241 | P2 | [ ] | Add golden parity tests and size/performance benchmarks comparing Python profiler outputs to Go outputs across collapsed/SVG/HTML/Jennifer samples. Track binary size, startup time, analysis throughput, and memory use. Targets: Wails v3 ≤20 MB, Fyne ≤40 MB (post-Python-removal). | T-239a–T-239g, T-240a–T-240f | User feature request | Go-vs-Python profiler validation |
...
| T-243 | P1 | [ ] | Multi-platform packaging — macOS `.app` (codesign + notarize), Windows `.exe` (MSIX or NSIS), Linux AppImage. Go cross-compile on a single host; for Wails follow `wails build`/`wails3 task package`, for Fyne use `fyne package`. Document Win10 WebView2 bootstrapper requirement if Wails. | T-240a | User feature request | Distributable installers |
| T-244 | P1 | [ ] | GitHub Actions CI matrix — `linux/macos/windows × {go test, golden parity (T-241), packaging smoke}`. Block merges on parity drift. | T-241, T-243 | User feature request | Native profiler CI gate |
| T-245 | P2 | [ ] | User docs (`docs/en/PROFILER_NATIVE.md` + `docs/ko/PROFILER_NATIVE.md`) — install/run/keyboard-shortcut/troubleshooting, including WebView2 prerequisite (Wails) or OpenGL/Mesa requirement (Fyne, Linux). | T-243 | User feature request | Bilingual native docs |
| T-246 | P2 | [ ] | Keyboard shortcuts + accessibility — Wails inherits the React app's existing a11y; Fyne needs explicit `widget.Focus`/aria-equivalents and a custom test-id strategy since `data-testid` doesn't apply. | T-240a | User feature request | Native a11y baseline |
| T-247 | P3 | [ ] | Auto-update channel decision — Sparkle (mac), Squirrel (win), AppImageUpdate (linux), or manual download notice. Defer until distribution model and signing certificates are settled. | T-243 | User feature request | Update strategy |
