...
| T-187 | P1 | [ ] | Wire the new profile formats into the (post-Phase-2) ProfilerAnalyzer page: add `flamegraph_svg`/`flamegraph_html` options to the `profileFormat` selector, extend the file-picker filters with `svg` and `html`, and surface `UNSUPPORTED_HTML_FORMAT` engine diagnostics through the new feedback panel. **Lands after Phase 2** so it ships once on the redesigned UI. | T-186, T-212 | User feature request | UI exposes SVG/HTML profiler inputs |
...
| T-193 | P1 | [ ] | Replace the ThreadDumpAnalyzer placeholder with a real (post-Phase-2) page: multi-file selection (drag many files / repeat browse), order-by-mtime toggle, per-snapshot language badge, persistence table, long-running/stuck findings, per-thread state timeline. Built on the redesigned tab/layout shell. **Lands after Phase 2**. | T-192, T-212 | User feature request | Multi-language multi-dump UI |
