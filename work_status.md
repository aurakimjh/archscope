...
Last updated: 2026-05-06 (Phase 1 web pivot — T-206 design note + T-207 Electron removal landed. `apps/desktop/` is gone (frontend already lived at `apps/frontend/`, so no rename was needed); `scripts/build-desktop.sh` removed; CI desktop job re-pointed to `apps/frontend`. New endpoints on the existing FastAPI server: `POST /api/files/select` (server-side absolute-path normaliser) and `WebSocket /ws/progress` (subscribe + cancel protocol), both backed by a new `ProgressRegistry` symmetric with the Wails T-240f task registry. `POST /api/analyzer/cancel` upgraded from no-op to task-id keyed real cancel. The Go/Native Profiler trail-end batch (T-240f / DebugLog UI / T-245 / T-246 / T-242 / T-247) shipped earlier in the day; that section has no open P1/P2 items left in the alpha scope. Final native binary 8.4 MB raw / 10 MB .app on darwin/arm64.)
