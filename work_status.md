...
### User-Driven UI/Feature Improvements and Multi-Language Diagnostics Expansion

Goal: trim wasted top-bar space, move language switching closer to navigation, drop the sandbox-incompatible native file `<input>`, expand the profiler analyzer to consume async-profiler/FlameGraph SVG and HTML inputs, and build a **language-agnostic multi-dump thread analysis framework** with parser plugins for Java jstack, Go goroutine dumps, Python py-spy/faulthandler, Node.js diagnostic-report, and .NET dotnet-dump.
...
- Playwright iTop+ automation (`Itop*Analyzer.java` in aitop-tools) — automation, not analysis.
- PowerPoint expansion of new outputs.

Design notes:

- The "multi-language" framework keeps language-specific cleanup and state-inference rules behind language-tagged enrichment plugins. The common engine never special-cases JVM frame names.
- A correlation bundle is a list of dumps from **one process at successive points in time**. Within a bundle, all dumps must share the same `source_format`/language. Mixing languages within one bundle is rejected with a clear error; mixing languages across separate bundles is fine.
- Auto-detection sniffs the first ~4 KB of each file; users can override with an explicit `--format` flag.

Items split into four tiers, ordered by priority:

1. **UI cleanup** (T-180~T-183) — independent prerequisite for clean analyzer pages.
2. **Profiler SVG/HTML** (T-184~T-187) — independent track building on the existing flame-tree contract.
3. **Common multi-language thread-dump framework** (T-188~T-193) — language-agnostic data model, parser plugin interface + auto-detection, Java reference plugin, correlation engine, CLI, UI.
4. **Per-language plugins and enrichment** (T-194~T-202) — Java enrichment first (reference), then Go/Python/Node.js/.NET in parallel.
5. **Cross-cutting wrap-up** (T-203~T-205) — language-agnostic latency finding, regression tests, bilingual docs.
...
| T-188 | P0 | [ ] | Define language-agnostic `ThreadSnapshot` and `ThreadDumpBundle` dataclasses in `models/thread_snapshot.py`. Fields: `snapshot_id`, `thread_name`, `thread_id`, `state`, `category`, `stack_frames: list[StackFrame]`, `lock_info`, `metadata: dict`. Bundle adds `dump_index`, `dump_label`, `captured_at`, `source_format` (`"java_jstack" \| "go_goroutine" \| "python_pyspy" \| "python_faulthandler" \| "nodejs_diagnostic_report" \| "dotnet_dump"`). `StackFrame` carries `function`, `module`, `file`, `line`, `language`. The existing JVM `ThreadDumpRecord` stays intact for the single-dump path; the multi-dump path uses the new model. | T-116 | User feature request | Multi-language record contract |
| T-189 | P0 | [ ] | Implement `parsers/thread_dump/registry.py` with a `ThreadDumpParserPlugin` Protocol (`format_id`, `detect(head: bytes \| str) -> bool`, `parse(path) -> list[ThreadSnapshot]`) and a registry that probes the first ~4 KB of each input to choose a parser. Reject bundles whose files resolve to mixed `source_format` values. Accept an explicit `format` override that bypasses detection. | T-188 | User feature request | Parser plugin interface + auto-detection |
| T-190 | P0 | [ ] | Port the existing JVM jstack parser into a `JavaJstackParserPlugin` registered with T-189 (detection signature: `Full thread dump` header or quoted-name + `nid=0x` line). Reuse `parsers/thread_dump_parser.py` internals so single-dump behavior is unchanged; the plugin only adapts output to `ThreadSnapshot`. | T-188, T-189 | User feature request | Java reference plugin |
| T-191 | P0 | [ ] | Implement multi-dump correlation in `analyzers/thread_dump_multi_analyzer.py` over `ThreadSnapshot`. For each `(thread_name, stack_signature)` group, compute occurrence count across `dump_index`-ordered snapshots and persistent-state runs. Emit findings `LONG_RUNNING_THREAD` (RUNNABLE/RUNNING for ≥3 consecutive dumps), `PERSISTENT_BLOCKED_THREAD` (BLOCKED/locked for ≥3). Output `AnalysisResult` with `type="thread_dump_multi"` and `summary.languages_detected`. Language-agnostic: never imports JVM-specific helpers directly. | T-188, T-190 | User feature request | Common correlation engine |
| T-192 | P1 | [ ] | Add CLI command `python -m archscope_engine.cli thread-dump analyze-multi --input <f1> --input <f2> ... [--format <id>]`. Auto-detect per file unless `--format` overrides; produce `thread_dump_multi` JSON with per-thread persistence table, top long-running stacks, and a state-transition timeline series. | T-189, T-191 | User feature request | Multi-dump CLI |
| T-193 | P1 | [ ] | Replace `ThreadDumpAnalyzerPage` placeholder with a real page: multi-file selection (drag many files / repeat browse), order-by-mtime toggle, per-snapshot language badge, persistence table, long-running/stuck findings, per-thread state timeline. Reuse `AnalyzerFeedback` panels. | T-192 | User feature request | Multi-language multi-dump UI |
...
49. `T-188 -> T-189 -> T-190 -> T-191 -> T-192 -> T-193`: language-agnostic record contract → parser plugin interface and auto-detection → Java reference plugin → common correlation engine → CLI → UI.
50. `T-190 -> {T-194, T-195}`: Java reference plugin must exist before the JVM-only AOP cleanup and JVM-only network categorization plugins attach to it.
51. `T-189 -> {T-196, T-198, T-200, T-202}`: other-language parser plugins (Go, Python, Node.js, .NET) can be built in parallel once the registry/auto-detection contract exists.
52. `T-196 -> T-197`, `T-198 -> T-199`, `T-200 -> T-201`: each language's parser lands before its language-tagged framework-cleanup and state-inference enrichment.
53. `{T-195, T-197, T-199, T-201} -> T-203`: cross-language latency finding only after each language has populated its `category` enrichment.
54. `{T-187, T-193, T-194, T-197, T-199, T-201, T-202, T-203} -> T-204 -> T-205`: lock new profiler and multi-language thread-dump behavior with tests before bilingual docs.
