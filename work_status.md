...
- Current execution focus: the post-`v0.3.5` Jennifer MSA drilldown follow-up
  plus the Chrome DevTools/V8 CPU profile slice, whose four design gates
  (T-558~T-561) all closed on 2026-07-20 and which is now implementation-ready. Long-term
...
and measurement-unit contracts were resolved on 2026-07-20 (T-558, T-559),
followed the same day by the desktop integration path (T-560, option A path
unification with a staged rollout) and the Phase 3 temporal-analysis redesign
(T-561, pre-collapse sampled-CPU-run contract). **All four Chrome profile
design gates are now closed**, so Phase 1 parsing, Phase 2 classification,
Phase 3 temporal findings (rendering deferred), and Phase 4 desktop
integration may all start.
...
coverage signals needs a measurement run on Windows. The T-572 fixture corpus
was built on 2026-07-20 (synthetic-first, in the shared asset repository) and
the T-575 HTTP session-Diff contract was fixed the same day (§12.4.1,
implementation deferred to Phase 4); T-573 and T-574 remain open as the
aggregator and Windows proxy-spike work. Other roadmap
candidates,
...
| T-560 | P0 | [x] | Define and implement one desktop integration path for profile evidence so Browser CPU analysis, the existing Profiler policy, Analysis Workspace, Diff, and Report Export all consume the same normalized parser output. | T-558, T-559 | Completed 2026-07-20 (§5.0.1): **option A path unification** — `parsers/profile.Parsed` is the single normalization point and `EngineService.AnalyzeProfileEvidence` → `analyzers/profile` → `profile_evidence` is the only canonical desktop path; the legacy `ProfilerService` format switch is frozen and new formats may never be added to it. Staged rollout: (1) Diff common loader `stacksFromParsed` + `DiffPage` auto-detect, (2) Browser page calls `AnalyzeProfileEvidence` only, (3) Workspace registration + Export Center consumption, (4) legacy page migration as a follow-up outside T-560 scope. Acceptance criteria AC-1~AC-4 cover CLI/desktop parity, all-format Diff, unchanged-export verification, and legacy freeze; execution items registered as Phase 4 items 18/22/23 |
| T-561 | P0 | [x] | Redesign Phase 3 temporal analysis: use Chrome trace task boundaries for true Long Task findings, or rename cpuprofile-only output to sampled CPU runs; define a pre-collapse ordered-series contract. | T-558, T-559 | Completed 2026-07-20 (§9.3.1): **both halves of the disjunction taken** — cpuprofile-only output is renamed to **sampled CPU runs** with finding `SAMPLED_CPU_HOTSPOT` (default 100ms, deliberately not RAIL's 50ms; "Long Task" wording banned), while true Long Task becomes `BROWSER_LONG_TASK` in Phase 4 item 21 using Chrome trace `RunTask` boundaries with CPU-sample attribution. Pre-collapse contract: `buildSampleRuns` consumes `ts_us` before `collapsedStacks` (aggregation path unchanged); run splitting on stack change, idle boundaries, and >10× median-delta gaps; bounded outputs `tables["cpu_sample_runs"]` (top-N) and `series["cpu_activity"]` (≤1,000 buckets) keep the envelope size-stable and are omitted for formats without `ts_us`. Invariants INV-T1~T5 add non-overlap, active-duration ceiling, flamegraph cross-check, and downsample/hitCount-only suppression. Flame-chart rendering (item 13) deferred pending demand |
