...
| T-302 | P0 | [ ] | 0 — foundation | `ThreadSnapshot` / `ThreadDumpBundle` / `StackFrame` / `ThreadState` enum coerce table — port `models/thread_snapshot.py`. Profiler track already has a partial copy; lift into the shared module and remove the duplicate. | T-301 | Shared thread model |
