...
suppress CPU overlays with explicit findings. T-583 remains `IN_PROGRESS` for
the Claude renderer handoff and narrow re-review.
**T-586 is complete**:
...
|---:|---|---|---|---|
| 1 | T-583 | P1 | `IN_PROGRESS` | X-RG1 review is `CONDITIONAL`; Codex B1/B2 remediation is complete, so Claude must render unavailable timestamp deltas before the narrow re-review |
...
- Current execution focus: T-583 / X-RG1 review remediation after the
  ...
  missing/non-numeric `v8_start_time_us` suppresses CPU overlays. Claude owns
  rendering an unavailable timestamp delta as `—`. After that handoff, run
  the narrow X-RG1 re-review and required Go/frontend regressions. T-582 /
...
1. **IN_PROGRESS — T-583 / X-RG1 HTTP/profile/server-evidence correlation:**
   ...
   fail closed. Next: Claude renders an unavailable `timestamp_delta_ms` as
   `—`; then rerun the narrow review plus
   `go test ./...`, `npm run test:state`, and `npm run build`. O1–O5 are
   recorded as non-blocking follow-up, not gate conditions. Obligations
