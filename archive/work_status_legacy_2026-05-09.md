...
Last updated: 2026-05-09 (**Go/Wails release baseline and large-file parsing follow-up**): `apps/profiler-native` has been folded into `apps/engine-native`; the active tree now owns the Go engine, profiler core, Cobra CLIs, and Wails desktop app in one module. Python/FastAPI/browser sources were moved to `archive/python-engine` and `archive/web-frontend-python` with `archive/README.md`; they are retained for reference and audits only. The evidence-bound AI interpretation module was ported to Go under `apps/engine-native/internal/aiinterpretation` (evidence registry, prompt construction, privacy redaction, response validation, local Ollama runtime, evaluation helpers). CI/workflows/scripts/docs were redirected to the Go/Wails path. Tooling installed locally: `task` 3.50.0 and `wails3` v3.0.0-alpha.84. Vite security warnings resolved by upgrading Vite to 8.0.11 and `@vitejs/plugin-react` to 6.0.1; `npm audit` now reports 0 vulnerabilities. Verification in this slice: `go test ./...` passed under `apps/engine-native`; `npm run build` passed; `GOCACHE=/tmp/aiservice-go-cache task package` passed with macOS outputs at 11 MB raw binary and 13 MB `.app`. `v0.3.0-rc1` has been rebuilt from the Go/Wails baseline and published as the latest GitHub release. The current risk is no longer the Electron-to-Wails pivot; it is large-file parser memory pressure, especially GC log timelines that produced 1.08 GB RSS and a 105 MB result from a 34 MB synthetic input. Older context retained below for historical traceability.
...
Active cycle (2026-05-09, revised after the `v0.3.0-rc1` release): the primary product line is the unified Go/Wails implementation under `apps/engine-native`. The former Electron, Python/FastAPI browser, and standalone `apps/profiler-native` tracks are historical context only. Near-term work now shifts from release mechanics to large-file parser reliability: cap GC result growth first, then move common line parsers and analyzers toward streaming/bounded aggregation.

Next execution queue:

1. **GC log output/memory cap** — add series downsampling and hard limits so large GC logs cannot generate result JSON proportional to every event.
2. **Streaming text line API** — add a common callback/iterator path that does not `ReadAll` and does not materialize `[]string`.
3. **Streaming parser/analyzer conversion** — convert access log, GC log, OTel JSONL, exception, and runtime-stack paths away from full line/record retention.
4. **Structured large-file guardrails** — add streaming or preflight limits for JFR JSON, Jennifer profile exports, thread-dump JSON/text plugins, profiler SVG/HTML inputs.
5. **Performance gates and documentation** — add reproducible synthetic large-file benchmarks and update the large-file policy in English/Korean docs.
6. **Distribution follow-up** — signing/notarization and frontend bundle code splitting remain later release-quality tasks after the parser risk is reduced.

Engine-UI Bridge decision: Wails v3 service bindings invoke the in-process Go engine inside the native desktop app. Local HTTP/FastAPI is retired from the active product path and retained only in `archive/` for reference.

AnalysisResult contract hardening scope: preserve the common Go `AnalysisResult` shell across parser, analyzer, exporter, CLI, Wails, and demo-runner surfaces. Strengthen runtime validation only when contract drift becomes observable in tests or generated Wails bindings.
...
| T-209 | P3 | [-] | **Obsolete after T-392 (2026-05-09).** This matrix verified the Python `archscope serve` browser deployment, which is now archived. Browser compatibility is retained only as historical documentation; active smoke coverage moves to the Wails desktop app and Go CLIs. | T-392 | Browser matrix retired |
| T-210 | P1 | [x] | Layout redesign verified after Phase 2 gap analysis. All 6 analyzer pages (`AccessLogAnalyzerPage`, `GcLogAnalyzerPage`, `ProfilerAnalyzerPage`, `ExceptionAnalyzerPage`, `JfrAnalyzerPage`, `ThreadDumpAnalyzerPage`) use the `FileDock` top-fixed file-selection strip + shadcn `Tabs/TabsList/TabsTrigger/TabsContent` content area. The 4 workspace pages (`ChartStudioPage`, `DemoDataCenterPage`, `ExportCenterPage`, `SettingsPage`) deliberately keep their own layouts since they aren't analyzer flows. Legacy `workspace-grid`/`summary-grid`/`chart-grid` selectors are removed in T-212 cleanup. | T-209 | User feature request | New layout shell |
| T-211 | P1 | [x] | Collapsible sidebar already shipped via `AppShell.tsx` + `AppSidebar.tsx`: icon-only `w-14` collapsed state with `transition-[width] duration-200`, localStorage persistence under `archscope.sidebar.collapsed`, `data-testid={\`nav-${item.key}\`}` preserved on every nav item, `<Tooltip>` on collapsed icons. This slice added a global `Cmd/Ctrl+B` keyboard shortcut in `AppShell` (VS Code convention, skips when typing in inputs/textareas/contenteditable). | T-210 | User feature request | Collapsible sidebar |
| T-212 | P1 | [x] | Tailwind v4 + shadcn/ui already in place across `AppShell` / `AppSidebar` / `TopBar` and every analyzer page (Tabs, Card, Button, DropdownMenu, Input, Tooltip, Separator from `@/components/ui/*`). This slice removed ~350 lines of dead legacy CSS from `apps/frontend/src/styles/global.css` (`.app-shell`, `.sidebar`, `.brand`, `.brand-mark`, `.nav-list`, `.nav-item`, `.main-panel`, `.topbar`, `.eyebrow`, `.locale-switcher`, `.pipeline`, `.page`, `.summary-grid`, `.chart-grid`, `.workspace-grid`, `.input-grid`, `.primary-button`, `.button-row`, `.drilldown-controls`, `.drilldown-panel`, `.breadcrumb`, `.flame-tree`, `.flame-node-*`) — every grep for those classes in `apps/frontend/src/**/*.{ts,tsx}` returned 0 hits. Bundled CSS dropped 73.86 → 69.58 KB. The remaining definitions (`.metric-card`, `.message-panel`, `.error-panel`, `.info-panel`, `.diagnostics-panel`, `.ai-findings-panel`, `.sortable-th`, `.field`, `.file-zone`, `.secondary-button`, …) are still consumed by `MetricCard.tsx` / `AnalyzerFeedback.tsx` / `SortableTable.tsx` / `ChartCustomizer.tsx`; migrating those to shadcn primitives is a follow-up cleanup, not a blocker for closing T-212. | T-211 | User feature request | Design-system migration |
| T-213 | P1 | [x] | Slim global topbar already in place (`TopBar.tsx`: `h-14 sticky top-0`, brand mark + theme + locale + settings dropdown, sidebar carries only the brand mark + nav). This slice wired the search input that was previously a dead placeholder: a typed search index over the 11 page labels (i18n) feeds a popover combobox with arrow-key navigation, Enter to jump, Esc/blur to close, mouse hover to highlight, and a `Cmd/Ctrl+K` global focus shortcut with the keyboard hint rendered as a `<kbd>⌘K</kbd>` chip. en/ko strings for the new "no match" copy added (`topbarSearchNoMatch`). | T-212 | User feature request | Slim topbar |
...
| T-234 | P2 | [x] | `_parse_smr_diagnostics` now collects every hex address inside the SMR block (skipping JVM bookkeeping lines like `_java_thread_list=` / `elements=` so those don't inflate the unresolved counter), records the `length=N` value, and a new `_post_process_smr` step cross-references each address with the parsed thread records' `tid` to produce `resolved` / `addresses_unresolved` lists with `resolved_count` / `addresses_unresolved_count`. The multi-thread analyzer surfaces both shapes — explicit zombie-tagged lines (`kind: tagged`) and unmatched address rows (`kind: address_unresolved`) — through `tables.smr_unresolved_threads`, and the `SMR_UNRESOLVED_THREAD` finding fires for either kind. New regression tests cover the JDK 8 style with no zombie marker, the JDK 17/21 `=>0x...` format, and the legacy explicit `unresolved zombie` line. 14 jstack-hardening tests pass. | T-228 | Implementation follow-up | Higher-confidence SMR correlation |
| T-235 | P3 | [x] | `_parse_text_class_histogram` now flags incomplete blocks via `incomplete=True` + `incomplete_reason` whenever (a) the histogram ends with a partial row that is missing the class-name column (`12345:  6789` with no name), (b) class rows were parsed but no `Total` summary line is found (the JVM always emits one — its absence implies truncation), or (c) the header was seen but no rows / totals could be parsed at all. The multi-thread analyzer surfaces these in `tables.class_histogram_incomplete`, bumps `summary.class_histogram_incomplete`, and emits a new `INCOMPLETE_HISTOGRAM` finding with the human-readable reason. Three regression tests cover the partial-row, missing-Total, and complete-block cases. | T-230 | Implementation follow-up | Better histogram parser diagnostics |
| T-236 | P2 | [x] | Backend `analyze_multi_thread_dumps` grew an `include_raw_snapshots: bool = False` knob so renderers can opt into the bigger raw-stack payload only when they need it; the default response stays small (carrier-pinning / SMR / native-method tables already sliced at `top_n`). New `apps/frontend/src/lib/usePagedRows.ts` hook (initial 50 rows, +100 per click, auto-resets when the source array reference changes) is wired into the `JvmSignalsTabs` carrier-pinning / SMR / native-method tables on `ThreadDumpAnalyzerPage.tsx`. New `<PaginationFooter>` component renders "Showing {visible} of {total} rows" + "Load more" button (en/ko `threadDumpPagedShowing` / `threadDumpPagedLoadMore` keys added). 50 000-virtual-thread dumps now mount 50 rows at a time instead of all at once. Type-check + Vite build green. | T-224, T-227 | User scalability follow-up | Safer large virtual-thread UX |
...
### Go Engine Full Conversion

Status: **complete as of 2026-05-09**. The complete active product line now ships from the unified Go/Wails module at `apps/engine-native`; the former profiler-native module, Python engine, and browser frontend are archived or folded into this tree. The inventory below is retained as the original conversion scope; the task rows that follow record the completed Go implementation and the web-server rejection.

**Initial scope inventory** (historical Python source tree, sized in Loc):

- **Models / common**: `models/{analysis_result, flamegraph, thread_snapshot, …}.py`, `common/{diagnostics, debug_log, redaction, file_utils, statistics}.py`. *Already Go (profiler track):* `FlameNode`, `ParserDiagnostics`, `DebugLog`, `RedactText`, `AnalysisResult` shell. *Remaining:* `ThreadSnapshot`/`ThreadDumpBundle`/`StackFrame`/`ThreadState`, statistics utilities, file_utils encoding helpers.
- **Parsers (16 top-level + 7 thread-dump plugins = 23 total)**. *Already Go:* `collapsed_parser`, `jennifer_csv_parser`, `svg_flamegraph_parser`, `html_profiler_parser`. *Remaining:* `access_log_parser`, `gc_log_parser` + `gc_log_header`, `exception_parser`, `otel_parser`, `jfr_parser`, `jfr_recording`, `dotnet_parser`, `go_panic_parser`, `nodejs_stack_parser`, `python_traceback_parser`, `thread_dump_parser`, plus the entire `parsers/thread_dump/` family (`registry`, `java_jstack`, `java_jcmd_json`, `go_goroutine`, `python_dump`, `nodejs_report`, `dotnet_clrstack`).
- **Analyzers (18)**. *Already Go (profiler track):* `profiler_analyzer` (collapsed/jennifer/svg/html), `profiler_breakdown`, `profiler_drilldown`, `profiler_diff`, `profiler_timeline` (timeline scope), `flamegraph_builder`. *Remaining:* `access_log_analyzer`, `exception_analyzer`, `gc_log_analyzer`, `jfr_analyzer`, `lock_contention_analyzer`, `multi_thread_analyzer`, `native_memory_analyzer`, `otel_analyzer`, `profile_classification`, `runtime_analyzer`, `thread_dump_analyzer`, `thread_dump_to_collapsed`.
- **Exporters (6)**. *Already Go:* `pprof_exporter`. *Remaining:* `csv_exporter`, `html_exporter`, `json_exporter`, `pptx_exporter`, `report_diff`.
- **AI interpretation (6 modules)** — completed in Go under `apps/engine-native/internal/aiinterpretation` by T-370.
- **Web server** — rejected for the active Go product by T-350. The FastAPI/browser path is archived; Wails service bindings and desktop task events are the active boundary.
- **CLI** — completed in Go with Cobra by T-360, including profiler and demo-site entry points.
- **Demo runner** — completed in Go under `apps/engine-native/internal/demosite` by T-380.

**Tier ordering** (each tier blocks the next; items inside a tier are parallel-safe):

| ID | Priority | Status | Tier | Task | Depends on | Output |
|---|---|---|---|---|---|---|
| T-301 | P0 | [x] | 0 — foundation | New `apps/engine-native/` Go module (`github.com/aurakimjh/archscope/apps/engine-native`) with five Tier-0 packages: `internal/diagnostics` (ParserDiagnostics builder + bounded sample lists matching Python's JSON shape), `internal/statistics` (Average / Percentile / BoundedPercentile reservoir sampler, math/rand/v2-based, deterministic seed), `internal/textio` (encoding detection + iter_text_lines with utf-8 / utf-8-sig BOM / cp949 (golang.org/x/text/korean) / utf-16-LE/BE BOM-and-null-parity / latin-1 universal fallback), `internal/models` (AnalysisResult envelope generalised beyond profiler — non-nil section maps, Findings always emitted, Metadata.Extra for analyzer-specific extensions), `internal/common` (lifted RedactText + DebugLog from the profiler track). 24 unit tests, all passing; `go vet` + `go build` clean. New `go-test-engine` matrix job in `.github/workflows/profiler-native.yml` covers ubuntu-latest / macos-14 / windows-latest with `go vet` + `go build` + `go test ./internal/... -race -count=1`. README documents the Tier-0..Tier-9 layout. | T-244 (parity gate) | Foundation Go module |
| T-302 | P0 | [x] | 0 — foundation | Ported `models/thread_snapshot.py` to `apps/engine-native/internal/models/`: `ThreadState` (string-typed enum + `CoerceThreadState` honoring all Python aliases — RUNNING/ACTIVE→RUNNABLE, PARKED/WAIT→WAITING, SLEEP/SLEEPING→TIMED_WAITING, IO→IO_WAIT, CHAN_RECEIVE/CHAN_SEND/SELECT→CHANNEL_WAIT, TERMINATED→DEAD, INITIALIZING→NEW; case/space/dash insensitive), `LockHandle` (lock_class as `*string` for null parity, wait_mode `omitempty`), `StackFrame` (Module/File/Line/Language as pointer types so unset fields emit `null`; `Render()` matches Python `f"{module}.{function} ({file}:{line})"` byte-for-byte), `ThreadSnapshot` (`NewThreadSnapshot` constructor with non-nil slice/map defaults; `StackSignature(depth)` joins top frames with `" \| "`, returns `(no-stack)` when empty, treats `depth<=0` as Python's `depth=5` default), `ThreadDumpBundle` (`NewThreadDumpBundle` constructor; `*time.Time` for captured_at). 9 new tests covering RUNNING / Sleeping / chan receive / "" / nonsense coercions, frame rendering matrix, depth limit, empty-stack signature, snapshot/bundle JSON shape, and lock-handle wait_mode omission. Profiler-native does not actually carry a ThreadSnapshot duplicate to remove (work_status guess; only profiler-specific FlameNode/TopStackRow live there). | T-301 | Shared thread model |
| T-310 | P1 | [x] | 1 — simple parsers | Ported `parsers/access_log_parser.py` to `apps/engine-native/internal/parsers/accesslog/` — three regex variants tried in order (nginx-with-response-time / combined / common), shares the parser across Python's seven format labels (nginx, apache, common, combined, ohs, weblogic, tomcat) where format is a diagnostic label only. Validates status (100..999), bytes (≥0), response_time_ms (finite, ≥0). New `internal/timeutil/` package for `ParseNginxTimestamp` (`02/Jan/2006:15:04:05 -0700`) + `MinuteBucket` (`2006-01-02T15:04:00-0700`, no-colon offset matching Python `%z`). Integrates with `textio.IterTextLines` (encoding auto-detect) and `diagnostics.AddSkipped` (per-reason histograms + bounded sample lists). 9 parser tests + 4 timeutil tests covering nginx/common/error paths, EMPTY_FILE warning, strict mode fail-fast, unsupported format rejection. | T-301 | `access_log_parser.go` + tests |
| T-311 | P1 | [x] | 1 — simple parsers | Ported `parsers/exception_parser.py` to `apps/engine-native/internal/parsers/exception/`. Java stack trace + cause chain (multi-`Caused by:`). Reused Python's `EXCEPTION_LINE_RE` byte-for-byte. Pointer types for `Timestamp`/`Message`/`RootCause`; `Message` distinguishes "no `:` delimiter" (None) from "empty after `:`" via `hasMessageDelimiter` because Go regexp collapses both to `""`. Skip / warning reasons (`NO_EXCEPTION_HEADER`, `INVALID_EXCEPTION_BLOCK`, `EMPTY_FILE`, `NO_EXCEPTION_BLOCKS`) ported verbatim. Stack flattens all `at` frames including caused-by frames. Timestamp parsing falls back through RFC3339Nano / RFC3339 / naive ISO / space-separated variants. 15 tests covering fixture happy path, analyzer-level case from `test_jvm_analyzers.py`, empty / no-block warnings, strict mode, MaxLines truncation, multi-cause chains, empty-message `Type:` distinction. | T-301 | `exception_parser.go` + tests |
| T-312 | P1 | [x] | 1 — simple parsers | Ported `parsers/gc_log_parser.py` + `parsers/gc_log_header.py` to `apps/engine-native/internal/parsers/gclog/` (kept as two files: `parser.go`, `header.go`). HotSpot unified GC log + G1 legacy + pre-G1 legacy + JVM info card. `Event` uses `*float64` / `*int` / `*time.Time` for Optional[T] so unset values JSON-marshal as `null`. `DetectFormat` follows Python heuristic exactly (including the side effect that bare `[GC cleanup ...]` alone doesn't classify as g1_legacy without a young/mixed pause marker). Header `_to_mb` uses 2-decimal rounding while parser uses 3-decimal — kept separate (`unifiedSizeToMB` vs `toMBPtr`). Strict mode flushes pending event before returning the error. Reasons (`NO_GC_FORMAT_MATCH`, `EMPTY_FILE`, `NO_SUPPORTED_GC_EVENTS`) verbatim. 32 tests covering all three formats, header extraction, strict mode, MaxLines, EMPTY_FILE / NO_SUPPORTED_GC_EVENTS warnings, metaspace companion merge logic. | T-301 | `gc_log_parser.go` + tests |
| T-313 | P1 | [x] | 1 — simple parsers | Ported `parsers/otel_parser.py` to `apps/engine-native/internal/parsers/otel/`. OTel JSONL trace/span parser. Used `map[string]any` via `json.Unmarshal` to top-level `any` rather than typed struct — Python is alias-tolerant across 12+ keys (snake / camel / dotted) with numeric/dict tolerance and falls back through `resource.attributes` / `attributes`. `formatNumber` collapses integral floats to `strconv.FormatInt` so JSON `12345` → `"12345"`, not `"12345.0"`. Booleans format as Python's capitalised `"True"/"False"`. `firstNonNilTruthy` mirrors Python's `body or message` truth-table for str/bool/float/dict/list. `FailedPattern = "OTEL_JSONL_LOG_RECORD"` exported for future debug-log parity. Diagnostic format string is `"otel_jsonl"`. 23 tests covering 9 field-alias cases, 5 diagnostics cases, 4 ParseLine-direct cases, 4 ParseFile feature cases (Strict, MaxLines, EMPTY_FILE, negative MaxLines), and a fixture-parity test against `examples/otel/sample-otel-logs.jsonl`. | T-301 | `otel_parser.go` + tests |
| T-314 | P1 | [x] | 1 — simple parsers | Ported `parsers/jfr_parser.py` + `parsers/jfr_recording.py` to `apps/engine-native/internal/parsers/jfr/` (`parser.go` for JSON; `recording.go` for the `os/exec` JDK CLI bridge). `os/exec.Command(cli, "print", "--json", jfrPath)` via `cmd.Output()` so stdout is captured and `*exec.ExitError` carries `Stderr` automatically — surfaces as `jfr print --json failed (exit N): <stderr>` matching Python's `RuntimeError` shape. `os.IsNotExist`-class start errors promote to `*CLIMissingError`. No timeout (Python's `subprocess.run` doesn't pass one either). `*JSONDecodeError` carrying `Message`/`Line`/`Column`/`Offset` — Go has no Python `json.JSONDecodeError` equivalent; line/column re-derived by walking the body once after `encoding/json` returns its byte offset. `toInt` rejects `bool` explicitly to match Python's `isinstance(v, bool)` guard. `previewJSON` normalises maps to deterministic sorted-key JSON before truncating to 500 chars. Reasons (`INVALID_JFR_JSON` / `INVALID_JFR_SHAPE` / `INVALID_JFR_EVENT`) verbatim. 28 tests, 27 PASS + 1 SKIP via `exec.LookPath("jfr")` for the integration case. | T-301 | JFR Go parsers + tests |
| T-315 | P1 | [x] | 1 — simple parsers | Ported four runtime stack parsers (`dotnet_parser.py`, `go_panic_parser.py`, `nodejs_stack_parser.py`, `python_traceback_parser.py`) to `apps/engine-native/internal/parsers/runtimestack/` as **one** package with one file per language + `types.go` for the shared `RuntimeStackRecord` / `IisAccessRecord` / `Options` types. All four emit the same record shape (only `dotnet` adds `IisAccessRecord`), so collapsing into one package preserves type identity for the downstream T-333 analyzers. `dotnet_parser` mixes two unrelated formats (.NET exception text + IIS W3C access logs) in a single pass with stateful field-schema capture on `#Fields:` directives. Python's go-panic `not line.startswith("\t")` guard is dead because `line.strip()` already removed any leading tab — kept verbatim for parity. Python traceback signature uses `stack[-1]` (deepest frame), opposite of every other runtime which uses `stack[0]` — mirrored faithfully. Node.js header regex has TWO type alternatives (bare `(?:Error\|Exception)` plus the more specific qualified form) so headers like literal `Error: foo` parse with `RecordType="Error"`. Optional regex groups use `FindStringSubmatchIndex` to distinguish "absent" from "matched empty" (Go RE2 collapses both to `""` with `FindStringSubmatch`). 18 tests across the four runtimes (6 / 4 / 4 / 4). | T-301 | Runtime parsers + tests |
| T-320 | P1 | [x] | 2 — thread-dump registry | Ported `parsers/thread_dump/registry.py` to `apps/engine-native/internal/threaddump/`. `Plugin` Go interface (FormatID / Language / CanParse / Parse) + optional `MultiBundlePlugin` capability for plugins that emit >1 bundle per file (concatenated jstack dumps). 4 KB head sniff via `textio.DetectFromBytes` + new `textio.DecodeBytes` public wrapper (universal latin-1 fallback). `Registry` with `Register` / `Plugins` / `Get` / `DetectFormat` / `ParseOne` / `ParseMany`. `ParseOptions` carries `FormatOverride`, `DumpIndex`, `DumpLabel`, `Labels` map. `*UnknownFormatError` (with HeadPreview truncated to 200 chars) and `*MixedFormatError` (with sorted format dict) match Python error semantics. `ParseMany` rejects mixed source formats by default; `FormatOverride` skips the check. Plugin order is registration-order (Python list-iteration parity). `DumpIndex` increments across all bundles; multi-bundle files get `#N` label suffix. Package-level `DefaultRegistry` singleton mirrors Python `DEFAULT_REGISTRY`. Note: profiler-native does NOT actually carry a registry duplicate (work_status guess; profiler-native is profiler-only). 14 tests covering Register / Plugins / Get / DetectFormat / ParseOne (override + sniff + unknown), ParseMany (mixed reject / override skip / multi-bundle / DumpIndex sequencing / plugin error surfacing), DefaultRegistry singleton, preview truncation. | T-302 | Registry + interface |
| T-321 | P1 | [x] | 2 — thread-dump registry | Ported `parsers/thread_dump/java_jstack.py` (924 lines, biggest of the six) to `apps/engine-native/internal/threaddump/plugins/javajstack/` split across 8 source files: `parser.go` (Plugin + ParseAll + frame regex + auto-register), `blocks.go` (thread-block splitter + state-from-header), `aop.go` (T-194: 6 CGLIB / Spring AOP / Hibernate proxy patterns in Python's exact order with Java-only guard), `state_inference.go` (T-195: NETWORK_WAIT / IO_WAIT regex with RUNNABLE-only guard), `monitors.go` (T-231: anchored lock-handle regexes + re-entrant dedup), `smr.go` (T-228 + T-234: SMR diagnostic section, address normalisation, post-processing unresolved counter), `histogram.go` (T-230 + T-235: class histogram + G1 heap), `virtual.go` (T-227 carrier pinning + T-229 native-method + virtual-thread detection). RE2 lacks `re.IGNORECASE` — case-insensitive regexes use inline `(?i)`. Lock-handle matching uses `FindStringSubmatchIndex` + `match[0]==0` to mirror Python `pattern.match(line)` semantics. SMR address normalisation (lowercase + lstrip zeros) shared across parsing and post-processing for cross-engine comparability. **60 tests** all green (T-194=8, T-195=7, T-227=8 incl. helpers, T-228/T-234=6, T-229=2, T-230/T-235=5, T-231=7, plus 17 end-to-end / parser tests). | T-320 | java_jstack Go plugin |
| T-322 | P1 | [x] | 2 — thread-dump registry | Ported `parsers/thread_dump/java_jcmd_json.py` to `apps/engine-native/internal/threaddump/plugins/javajcmdjson/` (`parser.go` 475 lines + `helpers.go` 433 lines). JSON-shaped `jcmd Thread.dump_to_file -format=json` output. Parsed as `map[string]any` with `json.Decoder.UseNumber()` because schema is too loose for Go structs — JDK 17/21/22 disagree on `name`\|`threadName`, `tid`\|`nid`\|`threadId`\|`id`, `stack`\|`stackTrace`\|`frames`, `state`\|`threadState`, `lockInfo`\|`blockedOn`\|`waitingOn`. `iterThreadObjects` walks via explicit stack with sorted-key iteration order so output is deterministic across Go's randomised map iteration (Python relied on dict insertion order). Decimal→hex `identityHashCode` coercion preserved (matches Python's `hex(int(raw_id))`). Snapshot ID format `java_json::0::<index>::<name>` kept identical to Python so cross-engine diffs stay byte-aligned. jstack helpers (`frameFromJstack`, proxy normalization, `inferJavaState`, `extractLockHandlesText`) inlined locally; lift to a shared `internal/threaddump/javacommon` package as a follow-up once T-321 lands. **19 tests** covering CanParse (4 cases), happy path matching Python's `test_java_jcmd_json_parser_routes_and_promotes_network_wait`, missing/malformed JSON, empty thread list, top-level array, lockInfo→LOCK_WAIT promotion + decimal→hex `identityHashCode`, `lockedSynchronizers` holds, structured frame objects, CGLIB proxy normalisation, virtual-thread metadata, RFC3339 `timestamp` alias, registry routing. | T-320 | java_jcmd_json Go plugin |
| T-323 | P2 | [x] | 2 — thread-dump registry | Ported `parsers/thread_dump/go_goroutine.py` to `apps/engine-native/internal/threaddump/plugins/gogoroutine/` (`parser.go` 376 lines + tests 303 lines). Framework cleanup: 6 regex pairs run sequentially against the qualified `module.function` string — five anchored entries map specific wrappers (`gin.HandlerFunc.funcN`, `gin.(*Engine).handleHTTPRequest`, `echo.(*Echo).ServeHTTP`, `chi.(*Mux).routeHTTP`, `fiber.(*App).Handler`) onto stable parents; the final entry strips any chain of trailing anonymous-closure suffixes (`.func1.func2…`) so receivers like `ServeHTTP` stay visible. State inference table: top-frame qualified-name → ThreadState with NETWORK_WAIT (netpoll / TCPConn.Read / persistConn.readResponse), LOCK_WAIT (semacquire / Mutex.Lock / RWMutex.Lock), CHANNEL_WAIT (gopark / chanrecv / chansend / selectgo), IO_WAIT (File.Read / bufio.Reader.Read / io.copyBuffer). Auto-registers in `init()` mirroring Python module-import side-effect. Reads via `os.ReadFile` + `textio.DetectFromBytes` + `textio.DecodeBytes` (not `IterTextLines`) so trailing-newline / blank-block semantics match Python's `splitlines()` exactly. **17 tests** all green. | T-320 | go_goroutine Go plugin |
| T-324 | P2 | [x] | 2 — thread-dump registry | Ported `parsers/thread_dump/python_dump.py` to `apps/engine-native/internal/threaddump/plugins/pythondump/` (`pythondump.go` 458 lines + tests 406 lines). Python source defines THREE explicit plugin classes (not two as task brief stated) — `PySpyPlugin`, `FaulthandlerPlugin`, `TracebackPlugin` — each self-registers with its own `format_id`. Kept as three plugin instances in one Go package so detection stays unambiguous. Shared helpers (`finalizePythonSnapshot`, `stripPythonBoilerplate`, `inferPythonState`) live alongside the three plugins. `finalizePythonSnapshot` always overwrites `Category` to `state.value` (Python parity) so faulthandler snapshots end up with `Category="UNKNOWN"` rather than `nil`. `stripPythonBoilerplate` returns a copy when stripping would empty the slice (Python `cleaned or list(frames)`). `readPythonDumpLines` falls back from `IterTextLines` to raw `os.ReadFile` so a malformed encoding never blocks parsing — Python uses `errors="replace"` and never raises here. CanParse uses `(?m)` (Python `re.MULTILINE`) for head sniff. **22 tests** (py-spy 6 + faulthandler 5 + traceback 3 + shared helpers 8). | T-320 | python_dump Go plugin |
| T-325 | P2 | [x] | 2 — thread-dump registry | Ported `parsers/thread_dump/nodejs_report.py` to `apps/engine-native/internal/threaddump/plugins/nodejsreport/` (`parser.go` 502 lines + tests 386 lines). Both Python plugins ported — `DiagnosticReportPlugin` (`nodejs_diagnostic_report`) for `process.report.writeReport()` + `SampleTracePlugin` (`nodejs_sample_trace`). Decoded into `map[string]any`; `asInt` helper mirrors Python's strict `isinstance(int)` filter (only integral floats accepted). `is_active` truthiness via Python-style `truthy` helper (false / 0 / "" / nil all skip). `libuv_handles` metadata stores raw `[]any` slice capped at 50; `header` metadata filtered to four whitelisted keys. Invalid JSON returns a structured empty bundle with `metadata["parse_error"] = "INVALID_NODEJS_REPORT_JSON"` (never an error — matches Python). `InferJSState` is exported so future T-201-style enrichment can re-run inference with out-of-band handle data. No `init()` registration (consistent with `accesslog`). No upstream Node fixture under `engines/python/tests/data/`; tests construct fixtures inline via `json.Marshal(canonicalReport())`, byte-identical to Python's `_REPORT`. **19 tests** all green. | T-320 | nodejs_report Go plugin |
| T-326 | P2 | [x] | 2 — thread-dump registry | Ported `parsers/thread_dump/dotnet_clrstack.py` to `apps/engine-native/internal/threaddump/plugins/dotnetclrstack/` (`plugin.go` 509 lines + tests 443 lines + interface assertion 16 lines). Both Python plugins ported — `Plugin` (`dotnet_clrstack`) for WinDbg `!clrstack` output + `EnvironmentStackTracePlugin` (`dotnet_environment_stacktrace`). Address-extraction strategy: SP/IP addresses on each clrstack frame matched by `^([0-9a-fA-F]+)\s+([0-9a-fA-F]+)\s+(.+)$` (after `TrimSpace`) but discarded — Python only keeps the call-site portion. Call site split at first `(` to isolate qualified name, partitioned at the LAST `.` for `module="MyApp.Service"` / `function="Process"`. Pure-token call sites keep `Module=nil`. CanParse uses `(?m)^OS Thread Id:\s+0x` (Python `re.MULTILINE`) anchored at line start so JVM/Go plugins don't lose their inputs. `managed_id` explicitly stored as `nil` in metadata when missing (Python `dict[str, Optional[str]]` JSON shape). Async state-machine normalisation runs `localFunctionRE` BEFORE `asyncStateMachineRE` (Python order — `<Outer>g__Inner|3_0` is more specific than `d__`). `splitLines` mimics `splitlines()` + per-line `rstrip("\r")` for Windows CRLF dumps. **23 tests** all green (covers all 7 Python tests + edge cases). | T-320 | dotnet_clrstack Go plugin |
| T-330 | P1 | [x] | 3 — analyzers | Ported `analyzers/access_log_analyzer.py` to `apps/engine-native/internal/analyzers/accesslog/`. 22-metric summary, per-minute timeline (requests / avg / p50 / p90 / p95 / p99 / status-class / error-rate / bytes / throughput), per-URL aggregates with status-mix + percentiles, top-N tables (by_count / by_avg / by_p95 / by_bytes / by_errors / status_codes), 5 finding rules verbatim from Python (HIGH_ERROR_RATE / ELEVATED_ERROR_RATE / SERVER_ERRORS_PRESENT / NON_STANDARD_HTTP_STATUS / SLOW_URL_P95 / ERROR_BURST_DETECTED). Banker's rounding (`math.RoundToEven`) so floats match Python's `round()` for non-edge cases. Static-vs-API URI heuristic identical to Python (`_STATIC_EXT_RE` + `_STATIC_PATH_HINT_RE`). Fixed `models.Metadata.Extra` JSON inlining via custom `MarshalJSON` so analyzer-specific keys (`format`, `analysis_options`) appear at the same level as `parser` / `schema_version` (was tagged `json:"-"` and never marshalled). 10 analyzer tests covering fixture parity (33.33% error rate), diagnostics propagation, `MaxLines` cap, time-range filter, finding emission (HIGH_ERROR_RATE + SERVER_ERRORS_PRESENT + SLOW_URL_P95), NON_STANDARD_HTTP_STATUS evidence, JSON shape with Extra inlined, 10k-record reservoir-sampler regression, status-family boundaries, classifier matrix. **Note:** the Python `test_analyze_access_log_reports_status_and_slow_url_findings` test is failing on `main` (expects `SLOW_URL_AVERAGE` while the code emits `SLOW_URL_P95`) — Go port matches the *code* not the test, since the test is the bug. | T-310 | `access_log_analyzer.go` + tests |
| T-331 | P1 | [x] | 3 — analyzers | Ported `analyzers/gc_log_analyzer.py` to `apps/engine-native/internal/analyzers/gclog/` (829 lines). Pause / heap series, per-collector stats, JVM Info card, percentile histogram bucketing, allocation/promotion rate, wall-time + throughput from uptime_sec. **7 finding rules** verbatim: `LONG_GC_PAUSE` (max≥100ms warning), `FULL_GC_PRESENT` (count>0 warning), `LOW_GC_THROUGHPUT` (wall>0 && <95% warning), `HIGH_P99_PAUSE` (≥200ms warning), `HUMONGOUS_ALLOCATION` (warning), `CONCURRENT_MODE_FAILURE` (critical), `PROMOTION_FAILURE` (critical). `parserNames` table maps `gc_format` → `metadata.parser` per Python `_PARSER_NAMES`; unknown falls through to `hotspot_unified_gc_log`. `orderedCounter` reproduces `Counter.most_common()` insertion-order tie-break. `formatFloat` keeps `.0` on integer uptimes (Python `str(10.0)` → `"10.0"`). 16 tests covering Python parity + percentile interpolation + finding rules + critical-severity assertions + JSON marshal smoke. | T-312 | `gc_log_analyzer.go` + tests |
| T-332 | P1 | [x] | 3 — analyzers | Ported `analyzers/jfr_analyzer.py` (514 lines) + `analyzers/native_memory_analyzer.py` (172 lines) to `apps/engine-native/internal/analyzers/jfr/` as separate files (`analyzer.go` + `native_memory.go`) within one package. Mode filter, time-window filter, heatmap strip, notable events for jfr; minimal inline flame-tree builder for native memory (avoids non-existent shared package). ISO timestamp parsing rejects fractional seconds >6 digits to match Python 3.9 `datetime.fromisoformat` strictness — `TestNanosecondTimestampsTreatedAsUnparseable` pins this. `notable_events` uses `sort.SliceStable` so insertion-order ties match Python's stable sort. `events_by_type` ties broken ascending by event_type. **Followed analyzer code** (schema_version `0.2.0`, `result.type=jfr_recording`); existing Python tests assert `0.1.0`/`poc=true` which the analyzer no longer emits — code wins, test is stale. 22 tests. | T-314 | JFR analyzer Go port |
| T-333 | P1 | [x] | 3 — analyzers | Ported `analyzers/exception_analyzer.py` (122 lines) + `analyzers/runtime_analyzer.py` (285 lines) to **two separate packages** `internal/analyzers/exception/` + `internal/analyzers/runtime/`. Justification: Python ships them as two files with no shared imports; consumed records differ (`exception.Record` vs `runtimestack.RuntimeStackRecord`); result envelopes differ. **Findings** ported verbatim: exception (`REPEATED_EXCEPTION_SIGNATURE` warning, `ROOT_CAUSE_PRESENT` info), runtime stack (`REPEATED_RUNTIME_STACK_SIGNATURE` warning, `STACKLESS_RUNTIME_RECORDS` info), runtime .NET-only (`IIS_5XX_PRESENT` warning, `DOTNET_EXCEPTIONS_PRESENT` info). `orderedCounter` per package matches `Counter.most_common` (count desc, ties by insertion order). Result/parser literals exported as constants for downstream callers. `Build*` split from `Analyze*` mirrors Python's two-tier API. .NET slow URLs preserve Python's quirky shape (Counter-of-times-not-counts, last-write-wins on dup method+uri keys; commented in source). 14 tests (exception 6 + runtime 8). | T-311, T-315 | Exception + runtime analyzers |
| T-334 | P1 | [x] | 3 — analyzers | Ported `analyzers/otel_analyzer.py` (532 lines) to `apps/engine-native/internal/analyzers/otel/` (976 lines + 581 test). Service-path graph: `spanRepresentatives` picks earliest record per `span_id` by `(timestamp, span_id)`; `childrenIndex` maps parent `span_id`→children sorted by record key; spans with parent not in rep set bucketed under "" (root); DFS pre-order yields service path, dedupes consecutive same-service. Falls back to timestamp-sorted record order when no span has a parent. **All 5 Python finding codes** verbatim with severities/evidence preserved. Error-record detection: explicit ERROR/FATAL/CRITICAL true; TRACE/DEBUG/INFO/NOTICE/WARN/WARNING false; otherwise body regex (`\berror\b`, `\bexception\b`, `\bfailed\b`, `\bfailure\b`, `\btimeout\b`, `\binsufficient_stock\b`, `\bgateway_timeout\b`). Result type `"otel_logs"` (Python literal, not `"otel"` from task brief). Cycle dedupe uses `\x00`-joined lex-smallest rotation. Manifest expectation matching is in spec but absent from Python code; ported only what exists. `orderedCounter[K]` generic helper. 13 tests. | T-313 | `otel_analyzer.go` |
| T-335 | P1 | [x] | 3 — analyzers | Ported `analyzers/thread_dump_analyzer.py` (126 lines) to `apps/engine-native/internal/analyzers/threaddump/`. JVM-only single-dump analyzer. State distribution counts `snapshot.State` (empty→`UNKNOWN`); category distribution counts pointer (nil/empty→`UNKNOWN`). Stack signatures use top-**3** frames (Python `record.stack[:3]`, NOT shared `StackSignature`'s default 5) joined with ` | `; empty stacks render as `(no-stack)`. `mostCommon` sorts by count desc, ties by first-seen insertion order to mirror `Counter.most_common`. Findings: `BLOCKED_THREADS_PRESENT` (warning when blocked>0), `WAITING_THREADS_DOMINATE` (info when waiting>runnable). `threads_with_locks` counts non-nil/non-empty `LockInfo`. Default `ParserDiagnostics` (parser=`java_thread_dump`, parsed_records=len(snapshots)) when caller omits. Schema `0.1.0`, type `"thread_dump"`. Frames rendered through `StackFrame.Render()` so module/file/line collapse uniformly — signatures differ textually from Python (Render adds space before `(`) but grouping behavior identical. 15 tests. | T-321 | `thread_dump_analyzer.go` |
| T-336 | P1 | [x] | 3 — analyzers | Ported `analyzers/multi_thread_analyzer.py` (858 lines, **biggest Tier-3**) to `apps/engine-native/internal/analyzers/multithread/` split across 4 files: `analyzer.go` (orchestration / envelope / series / tables), `correlator.go` (threadTimeline / threadObservation / run-length helpers / state-set defs), `findings.go` (persistence/latency/growing-lock/heuristic rules + assembly), `signatures.go` (JVM metadata: carrier pinning / SMR / class histogram). Cross-dump tracking: `threadTimeline` per thread name accumulates observations across bundles; persistence detection walks sorted observations tracking `(prev_index, current_sig, current_run)` for consecutive `dump_index == prev+1` matches with state filter. Uses Python's `_BLOCKED_STATES = {BLOCKED, LOCK_WAIT}`. Growing-lock keys per `lock_id` skipping `wait_mode in {object_wait, parking_condition_wait}`. **All 10 findings** verified: LONG_RUNNING_THREAD (3 tests), PERSISTENT_BLOCKED_THREAD (2), LATENCY_SECTION_DETECTED (2), GROWING_LOCK_CONTENTION (3), THREAD_CONGESTION_DETECTED (2), EXTERNAL_RESOURCE_WAIT_HIGH (1), LIKELY_GC_PAUSE_DETECTED (2), VIRTUAL_THREAD_CARRIER_PINNING (1), SMR_UNRESOLVED_THREAD (1), INCOMPLETE_HISTOGRAM (2). Threshold table: `consecutive_dumps=3` default, `congestion_waiting_ratio=0.10`, `external_resource_sleeping_ratio=0.25`, `gc_pause_unowned_block_ratio=0.50` — all strict `>`, banker rounding to 4 decimals. Iterates timelines in lex-sorted name order for deterministic output (Go map iteration is randomized). 28 tests covering all findings + 7 orchestration tests. | T-320 + all plugins | `multi_thread_analyzer.go` |
| T-337 | P1 | [x] | 3 — analyzers | Ported `analyzers/lock_contention_analyzer.py` (339 lines) to `apps/engine-native/internal/analyzers/lockcontention/` split across 3 files: `analyzer.go` (orchestration / envelope / contention table / summary / series / tables / diagnostics), `graph.go` (waits-for edges + DFS cycle detector), `findings.go` (rule emission). DFS cycle detection: recursive closure with three-color visited map (`0`=unseen, `1`=on-stack, `2`=done). Waits-for graph has out-degree 1 (each thread waits on at most one lock) so depth is bounded by chain length. When DFS hits `color[next] == 1`, the stack-index slice to end is the cycle. **Canonical-rotation dedupe**: cycles deduped by lexicographically-smallest rotation joined with `\x00` — Go equivalent of Python `min(rotations)` tuple-as-set-key. **Findings**: `LOCK_CONTENTION_HOTSPOT` (warning, one per contended lock, capped at `top_n=20`), `DEADLOCK_DETECTED` (critical, one per canonical cycle). Result type `"thread_dump_locks"` (Python source line 142 — task brief said `"lock_contention"` but Python wins). Cooperative wait modes (`object_wait`, `parking_condition_wait`) excluded from both contention-candidate filter and deadlock graph via shared `cooperativeWaitModes` set. `mostCommonName`/`topNNames` use frequency-DESC then lex-ASC for `Counter.most_common` parity. `sortedUnique` for `all_waiters`. 13 tests. | T-336 | `lock_contention_analyzer.go` |
| T-338 | P1 | [x] | 3 — analyzers | Ported `analyzers/thread_dump_to_collapsed.py` (117 lines) to `apps/engine-native/internal/analyzers/threaddumpcollapsed/`. Frame ordering: Python's `_collapse_snapshot` reverses the stack so root sits left, leaf right (caller-first); Go iterates `frames` in reverse and prepends thread name (when `IncludeThreadName`) as leftmost segment — byte-identical output. **Trimmed API**: Python's `convert_thread_dumps_to_collapsed` couples `DEFAULT_REGISTRY.parse_many` + folding; Go exposes `Convert([]ThreadDumpBundle, Options) map[string]int` only — callers run the registry separately, matching engine-native's parser/analyzer split. `SortedLines` ties broken lex-asc by stack key (Go map unordered; Python `most_common` uses insertion order — current Python tests only assert on the unique top count so this is compatible). No `AnalysisResult` emitter (Python source emits raw flamegraph fuel, not envelope). `renderFrame` deliberately omits file/line unlike `StackFrame.Render()` — collapsed format only carries callable identity. **Reuse from profiler-native**: none — `apps/profiler-native/internal/profiler/collapsed.go` is the *consumer* (parses collapsed→map), this port is the *producer* (emits collapsed from bundles). 9 tests. | T-321 | `thread_dump_to_collapsed.go` |
| T-339 | P2 | [x] | 3 — analyzers | Ported `analyzers/profile_classification.py` (104 lines) to `apps/engine-native/internal/analyzers/profileclassification/`. Config-loading via `//go:embed config/runtime_classification_rules.json` parity with Python's `importlib.resources` packaging — bytes ship inside the binary, `DefaultRules` parsed at package `init()` mirroring Python's module-level `DEFAULT_STACK_CLASSIFICATION_RULES`. `LoadRules(path)` reads from disk; `LoadPackagedRules()` re-parses embedded bytes returning a fresh slice (callers can mutate without poisoning DefaultRules); `BuiltinRules` Go-literal fallback used if embed parse fails. Tokens stored lower-cased at parse time matching Python `token.lower()`; haystack `strings.ToLower`'d once per `ClassifyStack` call (shipped tokens all ASCII so `ToLower` ≡ `casefold()`). Error messages match Python verbatim. `null` / scalars rejected explicitly via `looksLikeJSONArray` because Go's `json.Unmarshal([]byte("null"), &slice)` succeeds with nil slice (Python's `isinstance(value, list)` rejects None). Rule ordering preserved exactly (`Spring Batch` before `Spring Framework`, `HTTP / Network` before `JVM`) since first-match is the contract. 17 tests. | T-301 | Classification table loader |
| T-340 | P1 | [x] | 4 — exporters | Ported `exporters/json_exporter.py` (16 lines — tiny) to `apps/engine-native/internal/exporters/json/` (`exporter.go` 50 lines + tests 95 lines). `Write(path, result any)` and `Marshal(result any) ([]byte, error)` for filesystem and in-memory paths. Uses `json.NewEncoder` with `SetIndent("", "  ")` + `SetEscapeHTML(false)` — `<`, `>`, `&` round-trip verbatim and non-ASCII (`한글`) emits raw UTF-8 (Python `ensure_ascii=False` parity). `Encoder.Encode` appends `\n` automatically matching Python `json.dumps(...) + "\n"`. Creates missing parent directories with `os.MkdirAll`. 6 tests covering AnalysisResult round-trip, parent-directory creation, plain-dict payload, non-ASCII preservation, HTML char preservation, 2-space indent. | T-301 | `json_exporter.go` |
| T-341 | P1 | [x] | 4 — exporters | Ported `exporters/html_exporter.py` (397 lines) to `apps/engine-native/internal/exporters/html/` (`exporter.go` 647 lines + `assets/report.css` 77 lines + tests 340 lines). **Hybrid template strategy**: page shell (doctype, head, embedded `<style>`, `<title>`, body wrapper) goes through `html/template` so contextual auto-escaping covers title and CSS interpolation; body composed with `html.EscapeString` line-by-line — mirrors Python source pattern exactly (also uses `html.escape`). Composed body passed as `template.HTML` to prevent double-escaping. CSS embedded via `//go:embed assets/report.css` for fully self-contained single-file output (asserted by `TestOutputIsStandalone`). No external `<link>`/`<script src>`. Public API matches T-340: `Write` / `Marshal` + `WriteWithOptions` / `MarshalWithOptions` for `source_path` / `title` overrides. Map keys sorted for deterministic output (Go map iteration randomized; Python relies on insertion order — needed for snapshot tests + report-diff stability). `omittedString` sentinel mimics Python's "skip key whose value is `None`" rule. `toMap` accepts `models.AnalysisResult`, `map[string]any`, or any JSON-marshalable input by round-tripping through `encoding/json` with `UseNumber`. 12 tests. | T-340 | `html_exporter.go` |
| T-342 | P2 | [x] | 4 — exporters | Ported `exporters/pptx_exporter.py` (243 lines) to `apps/engine-native/internal/exporters/pptx/` (`exporter.go` 469 lines + tests 331 lines). **Hand-rolled OOXML** (no `unioffice` — stdlib only): `archive/zip` container + template strings via `fmt.Sprintf` for XML parts, mirroring Python f-string shape. Emitted parts: `[Content_Types].xml`, `_rels/.rels`, `ppt/presentation.xml` + `.rels`, `ppt/theme/theme1.xml`, `ppt/slideMasters/slideMaster1.xml` + `.rels`, `ppt/slideLayouts/slideLayout1.xml` + `.rels`, `ppt/slides/slide{1..N}.xml` + `.rels` per slide. `xml.sax.saxutils.escape` parity via small `xmlEscape` helper (only `&`, `<`, `>`). `archive/zip` with `zip.Deflate` matches Python `ZIP_DEFLATED`. `formatValue` collapses integral floats (`42.0` → `42`) so `total_requests: 42` reads naturally — `json.Unmarshal` makes everything `float64` otherwise. Findings capped at 6, summary metrics at 10 (Python parity). Source-files passed as comma-joined string. **NOT supported** (Python doesn't either): charts, images, animations, notes, speaker-notes — PowerPoint opens text-only decks fine. 12 tests. | T-340 | `pptx_exporter.go` |
| T-343 | P2 | [x] | 4 — exporters | Python source was a **stub** — 5 lines that raise `NotImplementedError("CSV export is planned for a later phase.")` with no `test_csv*` tests. Implemented the work_status-implied flattener ("summary/series flattening for spreadsheets") since matching the stub literally would ship a no-op binary. Ported to `apps/engine-native/internal/exporters/csv/` (`exporter.go` 422 lines + tests 354 lines). **Both APIs**: `Write` / `Marshal` (T-340 parity) emit a single CSV with section headers (`# summary`, `# series:<name>`); `WriteAll(dir, result)` directory mode for tools that can't parse multi-section files. Single-file is default; directory mode is opt-in. Flattening rules: summary → 2-column `key,value` sorted by key (scalars render natively `1.5` not `1.500000`; maps/slices JSON-encoded); series array-of-records → wide table with alphabetically-sorted union columns + blank cells for missing keys; series scalar slice → single `value` column; series map → 2-column `key,value`. `WriteAll` filenames sanitised to `[A-Za-z0-9._-]`. Reflection-based `asStringMap` so analyzers emitting `[]SomeStruct` or `map[string]int` flatten without forcing `[]any`. Sorted keys everywhere → deterministic output. `encoding/csv` handles RFC 4180 quoting automatically. 12 tests. | T-340 | `csv_exporter.go` |
| T-344 | P1 | [x] | 4 — exporters | Ported `exporters/report_diff.py` (97 lines) to `apps/engine-native/internal/exporters/reportdiff/` (`exporter.go` 390 lines + tests 491 lines). **Diff shape** mirrors Python verbatim: top-level `type="comparison_report"` + `source_files` / `created_at` / `summary` / `series` / `tables` / `charts` / `metadata`. Summary holds `label` (nullable), `before_type`, `after_type`, `changed_metrics`, `before_findings`, `after_findings`, `finding_delta`. `series.summary_metric_deltas` rows are `{metric, before, after, delta, change_percent}`. `series.finding_count_comparison` is `[{side:"before",...},{side:"after",...}]`. Metadata: `schema_version="0.1.0"`, `report_kind="before_after_diff"`, `before_created_at`, `after_created_at`. **Numeric strategy**: both — `delta` (absolute, 6dp) AND `change_percent` (4dp, null when `before==0`). Only emitted when BOTH sides numeric and metric appears in either summary; non-numeric / one-sided silently skipped. **Finding strategy**: count-only diff (Python parity). NOT severity- or code-aware — that would change AnalysisResult contract. Public API: `Diff(before, after) (any, error)` + `Marshal` + `Write` + `BuildComparisonReport` (path-based, Python parity). `toMap` round-trips through `encoding/json` so callers can pass `models.AnalysisResult` (with custom `Metadata.MarshalJSON`) or plain `map[string]any`. Different result types are NOT errors — types recorded in `summary.before_type`/`after_type`. 13 tests. | T-340 | `report_diff.go` |
| T-350 | — | [-] | **REJECTED 2026-05-07** | ~~`net/http` server replicating 14 FastAPI routes + `/ws/progress`~~. **Pivoted to desktop-only architecture** — no HTTP server needed. The agent-built T-350 implementation (`port/t-350-web-server` branch) was deleted. Engine-native stays a pure Go library (parsers / analyzers / exporters); desktop binary consumes it via Wails service bindings (see T-350' below). Decision driven by binary-size focus: HTTP server adds ~+1 MB dead weight to a desktop binary the user never `serve`s. | — | (rejected) |
| T-350' | P0 | [x] | 5 — desktop bindings | Done 2026-05-07. Created `EngineService` in `apps/profiler-native/cmd/archscope-profiler-app/engineservice.go` (495 lines) exposing 14 methods covering every analyzer (access_log / gc_log / jfr / native_memory / exception / runtime_stack[4 variants] / otel / thread_dump / multi_thread / lock_contention / collapsed / classify) and 5 exporters (json / html / pptx / csv / report-diff). **Architecture finding**: Go's `internal/` rule blocks cross-module imports of `internal/...` even with `go.work`, so a thin re-export facade `apps/engine-native/api/` (213 lines, type aliases + var assignments, **zero logic**) was added as the integration boundary. New convention: external callers import from `api`, never directly from `internal/...`. Side-effect imports of all 6 thread-dump plugins live in the facade so any caller automatically registers them. Sync vs async split: long-running multi-file analyzers (multi_thread, lock_contention) use `taskRegistry`-backed async pattern with `engine:done` / `engine:error` / `engine:cancelled` events; everything else returns synchronously. Reuses the existing `taskRegistry` from `profilerservice.go`. New `go.work` (root) + `replace` line in `apps/profiler-native/go.mod`. 11 tests including JSON export round-trip, async TaskID returns, multi-variant runtime stack. **Binary size**: 21 MB unstripped / 14 MB with `-ldflags="-s -w"`. Wails baseline alone (no engine wiring) is already 19 MB unstripped — engine wiring adds ~+2 MB unstripped. The 12 MB stripped target is missed by ~2 MB; mitigation options: `task package` (adds -trimpath + codesigning), UPX compression (~50-70% reduction → 5-7 MB), or trim unused thread-dump plugins via build tags. The committed 8.4 MB binary at `bin/archscope-profiler` was stale (not a current `go build` output). **Wails TS binding generation**: not run (wails3 CLI not on PATH this session); will be picked up when developer runs `task generate:bindings`. | T-330–T-339, T-340–T-344 | Wails service exposing engine-native to desktop UI |
| T-351 | — | [-] | **REJECTED 2026-05-07** | ~~Static React mount via `embed.FS` for the HTTP-served bundle~~. Killed alongside T-350. Wails already serves React from its system webview; no Go HTTP serving needed. | — | (rejected) |
| T-351' | P1 | [x] | 5 — desktop UI | Done 2026-05-07 in two phases via parallel agent dispatch. **Phase 1**: foundation + AccessLog as the canonical pattern — `bridge/engine.ts` + `bridge/types.ts` (manual TS wrapper for `EngineService` since `wails3 generate bindings` not on PATH; will be drop-in replaceable when run), Tailwind v4 + Radix + echarts deps added, lifted shadcn UI primitives (button/card/input/tabs) + MetricCard / ChartPanel / AnalyzerFeedback + `WailsFileDock` (uses `Dialogs.OpenFile` from `@wailsio/runtime` — no Go-side change), end-to-end `AccessLogAnalyzerPage.tsx` (779 lines). **Phase 2**: 4 analyzer pages ported in parallel — `GcLogAnalyzerPage` (773 lines, 7 GC findings + pause/heap series + JVM info card), `JfrAnalyzerPage` (851 lines, dual-mode jfr_recording + native_memory in one page), `ExceptionAnalyzerPage` (~500 lines, type/root-cause/signature aggregation + REPEATED_EXCEPTION_SIGNATURE / ROOT_CAUSE_PRESENT findings), `ThreadDumpAnalyzerPage` (~1800 lines, 3 modes — single/multi/lock-contention with async `engine:done`/`error`/`cancelled` event subscription for the async ones). All 4 phase-2 agents conflicted on shared files (Sidebar / App / bridge/types / i18n); resolved manually by accumulating both sides (additive — no key collisions). Build verification: `npm run build` produces `dist/assets/index.js` ~1.54 MB / 486 KB gzip. `go build ./...` clean. Sidebar nav now has 7 entries: profiler / diff / access_log / gc_log / jfr / exception / thread_dump / settings. **Wails TS binding generation**: still skipped (CLI not on PATH); manual `bridge/` shim is the dev-time bridge, will be regenerated via `task generate:bindings` on a developer machine. | T-350' | Full-feature desktop UI |
| T-352 | P3 | [x] | 5 — frontend cleanup | **Closed 2026-05-09 by Go/Wails consolidation.** The legacy React browser app was moved from `apps/frontend/` to `archive/web-frontend-python`; the Python web static path moved with `archive/python-engine`. The active Wails frontend now lives under `apps/engine-native/cmd/archscope-profiler-app/frontend/`, so no shared `packages/ui-shared/` extraction is needed for the current release. | T-392 | Frontend archived; Wails UI is active |
| T-360 | P2 | [x] | 6 — CLI | Done 2026-05-07. Refactored `archscope-engine` from flag-based switch (489 lines) to **Cobra command tree** with 12 top-level groups + 21 leaf commands mirroring Python typer surface byte-for-byte: `access-log analyze`, `gc-log analyze`, `jfr analyze-json` / `analyze-native-memory`, `exception analyze`, `otel analyze`, `thread-dump analyze`/`analyze-multi`/`analyze-locks`/`to-collapsed`, `nodejs analyze`, `python-traceback analyze`, `go-panic analyze`, `dotnet analyze`, `report html`/`pptx`/`csv`/`json`/`diff`, plus `profiler` and `demo-site` as **stub groups** (501-style messages pointing at apps/profiler-native and T-380 respectively; visible in `--help` for discoverability). New `report` group wires the T-340..T-344 exporters: html / pptx / csv / json (round-trip / pretty-print) / diff (loads two AnalysisResult JSONs and writes the comparison report). Files split per group: `cmd_{accesslog,gclog,jfr,exception,otel,threaddump,runtime,report,demosite,profiler}.go` + `helpers.go` shared (parseTimeFlag / readJSONFile / writeJSONResult / splitCommaSeparated for repeatable `--in p1,p2` and `--in p1 --in p2` parity-gate forms). 13 parity workflow YAML invocations updated to new Cobra names. 13 subprocess smoke tests rewritten + still pass. **Only one new dep**: `github.com/spf13/cobra v1.10.2` + 2 transitives (pflag + mousetrap); no viper, no urfave/cli. `go.work.sum` lockfile tracked. Flag convention: `--in` (input path), `--out` (output path; `-` = stdout). `RunE` everywhere so errors flow through Cobra's envelope; `SilenceUsage`/`SilenceErrors` on root keeps error output single-line. | T-330–T-344 | Headless CLI for CI / scripting |
| T-370 | P2 | [x] | 7 — AI interpretation | **Done 2026-05-09.** Ported `ai_interpretation/{evidence,validation,prompting,runtime,privacy,evaluation}.py` to `apps/engine-native/internal/aiinterpretation`: evidence registry and reference parsing, evidence-bound prompt construction, privacy redaction, response validation, local Ollama runtime, and golden-scenario evaluation helpers. Optional feature remains off by default and has no Python runtime coupling. | T-336 | AI interpretation Go module |
| T-380 | P2 | [x] | 8 — demo runner | Done 2026-05-07. Ported `demo_site_runner.py` (854 lines) + `demo_site_mapping.py` (90 lines) to `apps/engine-native/internal/demosite/` split across 5 files for review tractability: `mapping.go` (157 lines — AnalyzerTypeMapping + LoadAnalyzerTypeMappings + FindAnalyzerTypeMapping + CommandForMapping/InputOptionForMapping with FormatOverrides), `runner.go` (603 — RunOptions / DemoAnalyzerRun / DemoScenarioRun / DiscoverDemoManifests / RunDemoSiteManifest / command builder / scenario summary), `analyze.go` (276 — analyzer dispatch table + annotateDemoMetadata + OTel manifest validation), `baseline.go` (145 — baseline before/after diff via reportdiff exporter), `index.go` (458 — RenderDemoIndex + WriteTopLevelIndex). **Manifest schema decision**: JSON-only to stay stdlib-only; Python's YAML support documented as user must convert via `yq -o=json` before invoking. Cobra stub at `cmd_demosite.go` replaced with **3 real commands**: `demo-site list --manifests <dir>` (sorted-stdout discovery), `demo-site run --manifest <path> --out <dir>` (single-manifest end-to-end with optional `--baseline`/`--mapping`/`--no-pptx`), `demo-site run-all --manifests <dir> --out <root>` (discover + run each + top-level index.html). **Profiler dispatch divergence**: engine-native ships only `jenniferprofile` (wall/collapsed profiler lives under apps/profiler-native per cmd_profiler.go stub); profiler_collapsed format with non-jennifer input emits per-file "not available in engine-native" error rather than scenario-abort, matching Python's try/except shape. **Baseline recursion**: when `BaselineManifestPath` is set + `<output_root>/<data_source>/normal-baseline/` absent, runner triggers a baseline run with `WritePPTX=false` and no nested baseline, mirroring Python. **Index HTML**: inline `strings.Builder` + `html.EscapeString` (same shape as Python's f-string + html.escape so side-by-side diff stays readable). 11 demosite unit tests + 15 CLI subprocess smoke tests (incl. new `TestCLIDemoSiteList`) all pass under `-race -count=1`. Manifest fixtures absent in repo so tests write inline JSON; access-log fixture uses existing `examples/access-logs/sample-nginx-access.log` (asserts summary.total_requests == 6 matching Python parity). | T-340–T-344, T-360 | `demo-site` CLI subcommand |
| T-390 | P1 | [x] | 9 — CI parity gate | Done 2026-05-07. Extended `.github/workflows/profiler-native.yml` `python-go-parity` job from 1 surface (access-log, plus the existing collapsed-profiler step) to **11 surfaces**. Added 10 new Go CLI subcommands to `apps/engine-native/cmd/archscope-engine/main.go` (still flag-based — Cobra is T-360 territory): `gclog`, `jfr`, `exception`, `runtime --variant`, `otel`, `threaddump`, `multithread`, `lockcontention`, `collapsed`. Each takes `--in`/`--out` and emits `models.AnalysisResult` JSON via the same `MarshalIndent` body as `accesslog`. Multi-file analyzers (`multithread`, `lockcontention`, `collapsed`) accept repeatable / comma-separated `--in` and parse via `threaddump.DefaultRegistry.ParseMany` (auto-registers all 6 plugins via side-effect imports). 11 new workflow steps mirror the access-log parity step pattern: build Go side, run Python equivalent (typer subcommands `gclog analyze`, `jfr analyze-json`, `exception analyze`, `otel analyze`, `thread-dump analyze`/`analyze-multi`/`analyze-locks`/`to-collapsed`, `nodejs analyze`, `python-traceback analyze`, `go-panic analyze`, `dotnet analyze`), then a Python heredoc loads both JSONs and asserts invariants (type / summary numerics / metadata.parser / finding code-set / series+tables key sets / diagnostics counts). Each ends with `print("OK ...")` for scannable CI logs. **All 11 verified locally** before commit (mirror script `/tmp/run_all_parity.sh`); headline counts match (GC events 2/2, JFR events 3/3, exceptions 2/2, otel records 4/4, threads 2/2, unique threads 2/2, unique locks 1/1, .NET IIS requests 3/3). Known minor divergence: JFR Python doesn't emit `findings` (Go does) — comparison uses `metadata.get("findings", [])` fallback so empty-list parity holds. New `cmd/archscope-engine/main_test.go` (209 lines): subprocess smoke tests build the binary into a temp dir and assert each subcommand parses + `result.type` matches expected. 14 Go test cases pass under `-race -count=1`. | T-330–T-344 | Full-engine parity gate |
| T-391 | P1 | [x] | 9 — packaging | **GO TRACK — DONE 2026-05-07. PRIMARY RELEASE ARTIFACT** = single Wails desktop binary covering full feature surface via T-350' / T-351'. **Size**: darwin/arm64 raw 11 MB, `.app` 11 MB, `.dmg` 5.3 MB (UDZO compressed) — all under budget. **Version bumped** to v0.3.0-rc1 across `build/darwin/Info.plist`, `build/windows/info.json` (also fixed Wails default ProductName "Probe" → "ArchScope Profiler"), `build/linux/nfpm/nfpm.yaml` (also `name: probe → archscope-profiler` + dependency paths), `build/linux/desktop` (template), Wails frontend `package.json`. **DMG packaging**: new `darwin:package:dmg` Taskfile target — pure stdlib `hdiutil create -format UDZO`, no third-party deps, drag-to-Applications symlink baked in. Verified locally produces a CRC32-valid mountable .dmg (5.3 MB / 11 MB raw). **Codesign + notarize infra**: new `darwin:codesign:developer` (Developer ID + hardened runtime + entitlements + timestamp + verify) and `darwin:notarize` (zips + submits via `xcrun notarytool` with keychain-profile or APPLE_ID/TEAM/APP_PASSWORD trio + staples). Both gated by env vars; absent → fall back to ad-hoc. **GitHub Release workflow** (`.github/workflows/release.yml` rewrite — old Electron version replaced): fires on `v*` tags or `workflow_dispatch`, builds across darwin-{arm64,amd64} / windows-amd64 / linux-amd64 matrix, imports Developer ID cert from base64-encoded p12 secret into temp keychain, signs + notarizes when secrets present (graceful fallback when absent), generates per-platform installers, computes SHA256SUMS, creates GitHub Release with auto-generated notes (pre-release flag auto-set when tag contains `-rc`/`-alpha`/`-beta`). **PACKAGING.md** rewritten with v0.3.0 invocations + reproducible local build command + size budget table + secret list. To cut a release: `git tag v0.3.0-rc1 && git push origin v0.3.0-rc1`. | T-350', T-351' | Single desktop binary |
| T-392 | P3 | [x] | 9 — decision | **Decided and executed 2026-05-09.** Python/FastAPI/browser runtime is retired from the active tree and archived, not deleted, under `archive/python-engine` and `archive/web-frontend-python`. Go/Wails is the primary runtime and distribution path. Archived sources remain for audits, reference, and emergency parity checks. | T-390 | Python archived; Go/Wails primary |

### Go Engine Large-File Parsing Performance Follow-up

Goal: reduce peak memory and result size for large logs/files in the active Go engine. This section reflects the 2026-05-09 audit of `apps/engine-native` after the Go/Wails consolidation. Synthetic measurements showed that collapsed profiler parsing is already memory-bounded, while several log/JSON parsers still materialize whole files. The immediate blocker is GC log analysis: a 34 MB synthetic GC log produced ~1.08 GB RSS and a 105 MB result because event timelines are emitted without downsampling.

Audit baseline:

| Input path | Synthetic size | Time | Peak RSS | Output size | Risk |
|---|---:|---:|---:|---:|---|
| Access log analyzer | 30 MB / 300k lines | 0.51s | 173 MB | 10 KB | Full line + record retention; acceptable at 30 MB, risky at GB scale |
| OTel JSONL analyzer | 31 MB / 200k lines | 0.25s | 147 MB | 6 KB | Full line + trace record retention; trace grouping needs bounded strategy |
| GC log analyzer | 34 MB / 300k lines | 1.51s | **1.08 GB** | **105 MB** | P0: unbounded timeline/result growth |
| Collapsed profiler | 29 MB / 500k lines | 0.26s | 16 MB | 6.8 KB | Good streaming baseline |

Execution order:

1. Fix GC result growth first. This is the only audited path that turns a modest input into a huge output and >1 GB RSS.
2. Add a common streaming text API before broad parser rewrites, so access/GC/OTel/exception/runtime-stack fixes share one encoding and diagnostics policy.
3. Convert analyzers after parsers, because parser streaming alone still leaves `[]Record`/`[]Event` retention in several builders.
4. Address structured full-read formats with size guards and targeted streaming, rather than forcing every JSON/HTML/SVG path through the same line iterator.

| ID | Priority | Status | Task | Depends on | Source | Output |
|---|---|---|---|---|---|---|
| T-393 | P0 | [ ] | Add GC analyzer `MaxSeriesPoints` limits and deterministic downsampling for `pause_timeline`, heap/young/old/metaspace, allocation, and promotion series while preserving exact summary metrics, percentile findings, and top event tables. | T-331, T-391 | 2026-05-09 large-file audit | Bounded `gc_log` result size and lower RSS |
| T-394 | P0 | [ ] | Add a reproducible GC large-file regression benchmark that asserts output size stays bounded for synthetic 300k+ event logs and records timing/RSS guidance for local verification. | T-393 | 2026-05-09 large-file audit | GC performance regression guard |
| T-395 | P1 | [ ] | Add a common streaming text API in `internal/textio` such as `ForEachTextLine(path, encoding, callback)` with BOM/encoding fallback, line numbers, cancellation hooks, and diagnostic context support without returning `[]string`. | T-225 | 2026-05-09 large-file audit | Shared line streaming primitive |
| T-396 | P1 | [ ] | Convert access log, GC log, OTel JSONL, exception, and simple runtime-stack parsers away from `IterTextLines`/`ReadAll` toward the streaming text API. Keep compatibility wrappers only for tests and small-file callers. | T-395 | 2026-05-09 large-file audit | Parser memory no longer proportional to full line slices |
| T-397 | P1 | [ ] | Refactor analyzer builders so access log, GC log, and OTel can aggregate from streams instead of requiring full `[]Record`/`[]Event` retention; OTel trace grouping must use bounded per-trace summaries, caps, or spill strategy. | T-396, T-393 | 2026-05-09 large-file audit | Memory-bounded analyzer paths |
| T-398 | P2 | [ ] | Add JFR JSON large-file controls: stream the events array where practical, expose event/time/stack-depth filters, and preflight/warn when direct JSON loading would exceed the supported local memory budget. | T-395 | 2026-05-09 large-file audit | Safer JFR JSON ingestion |
| T-399 | P2 | [ ] | Replace Jennifer profile global `ReadFile` + `FindAllStringIndex` TXID splitting with streaming block segmentation so one transaction block can be parsed and released at a time. | T-395 | 2026-05-09 large-file audit | Bounded Jennifer profile parser |
| T-400 | P2 | [ ] | Audit and convert high-risk thread-dump plugins that duplicate entire files or all sections in memory, including Java jstack section splitting, Go goroutine dumps, Node sample traces, .NET clrstack, and Python dump fallbacks. | T-395 | 2026-05-09 large-file audit | Large thread dump safety pass |
| T-401 | P2 | [ ] | Reduce profiler HTML/SVG memory copies: avoid `string(payload)` where `bytes.Reader` works, add size preflight for self-contained HTML payloads, and document when full HTML parse remains intentionally bounded. | T-395 | 2026-05-09 large-file audit | Lower parser copy overhead for visual profiler inputs |
| T-402 | P2 | [ ] | Update English/Korean parser and performance docs with the Go engine large-file policy: warning thresholds, stream-only paths, `max-lines`/time filters, output downsampling, and UI messaging for very large inputs. | T-393, T-397 | 2026-05-09 large-file audit | Current large-file operations guide |
...
**Go Engine Full Conversion — Tier-0 to Tier-9:**

63. `T-301 -> T-302`: foundation models + ThreadSnapshot family before any parser depends on them.
64. `T-301 -> {T-310, T-311, T-312, T-313, T-314, T-315}`: every Tier-1 simple parser only needs the foundation models.
65. `T-302 -> T-320 -> {T-321, T-322, T-323, T-324, T-325, T-326}`: Tier-2 thread-dump registry first, then the per-language plugins land in parallel.
66. `T-310 -> T-330`, `T-311+T-315 -> T-333`, `T-312 -> T-331`, `T-313 -> T-334`, `T-314 -> T-332`: each Tier-3 analyzer waits on its parser.
67. `T-321 -> T-335`, `T-320 + all plugins -> T-336 -> T-337`, `T-321 -> T-338`: thread-dump analyzers fan out from the registry once enough plugins ported.
68. `T-301 -> T-340 -> {T-341, T-342, T-343, T-344}`: JSON exporter is the foundation for the rest of the report family.
69. ~~`T-330..T-339 + T-340..T-344 -> T-350 -> T-351`~~: **REJECTED 2026-05-07 desktop pivot.** Replaced by `T-330..T-344 -> T-350' -> T-351'`: Wails service bindings expose engine-native to the desktop UI; T-351' extends the desktop frontend with all analyzer pages.
69a. `T-351' -> T-352`: frontend cleanup is complete; the retired browser UI lives in `archive/web-frontend-python`, and the Wails frontend is the only active UI.
70. `T-330..T-344 -> T-360`: CLI subcommands wrap each analyzer/exporter for headless / CI / parity-gate use (no longer fronts a web server).
71. `T-336 -> T-370`: AI interpretation is complete in Go under `apps/engine-native/internal/aiinterpretation`.
72. `T-340..T-344 + T-360 -> T-380`: demo runner reuses the report family and the CLI wrappers.
73. `T-330..T-344 -> T-390 -> T-391 -> T-392`: CI parity gate, packaging, and Python retirement decision are complete. The Python-removal sweep archived Python/browser sources instead of deleting them. Active stream is now release validation: remote CI check -> `v0.3.0-rc1` tag -> desktop smoke -> bundle optimization if needed.
...
| Web file-selection model | T-206 | Historical decision for the archived FastAPI/browser path. Active Wails file selection uses native dialogs and Go service bindings. |
| Web transport for engine progress and cancellation | T-206, T-207 | Historical decision for the archived FastAPI/browser path. Active Wails progress/cancellation uses desktop task events, not HTTP/WebSocket. |
...
| Web app directory rename | T-207 | Obsolete. The active frontend is `apps/engine-native/cmd/archscope-profiler-app/frontend/`; the retired browser UI is archived under `archive/web-frontend-python`. |
...
| Native profiler GUI framework | T-240a | **Decided/executed 2026-05-09: Wails v3.** The active UI lives under `apps/engine-native/cmd/archscope-profiler-app/frontend/` and keeps the React/Tailwind/shadcn/D3 approach inside a system webview. Webview dependencies — macOS WKWebView, Win10 WebView2, Linux WebKitGTK — are accepted trade-offs. Fyne and other Go-native widget toolkits remain rejected because they would require a parallel UI implementation. |
| Go full-engine module location | T-301 | **Decided/executed 2026-05-09: unified `apps/engine-native/` Go module** (`github.com/aurakimjh/archscope/apps/engine-native`). The profiler core, engine analyzers, Cobra CLIs, and Wails app now live in this single module. |
| Python engine retirement window | T-392 | **Decided/executed 2026-05-09:** Python/FastAPI/browser sources are retired from active development and archived, not deleted, under `archive/python-engine` and `archive/web-frontend-python`. Go/Wails is the primary runtime and distribution path. |
| Go HTTP / WebSocket libraries | T-350 | **OBSOLETE 2026-05-07**: HTTP server rejected with the desktop-only pivot. The agent-built `port/t-350-web-server` branch (`net/http` + `coder/websocket`) was deleted unmerged. Decision retained for history. |
| **Go binary deployment model** | T-350, T-391 | **Decided/executed 2026-05-09: desktop-first, Go/Wails primary.** The release path ships the Wails v3 desktop app (`archscope-profiler`) plus thin Go CLIs from the unified `apps/engine-native` module. HTTP server / `embed.FS` browser deployment remain rejected for the active binary. |
| UI in the Go conversion scope | T-351 | **Revised/executed 2026-05-09.** The UI is not rewritten in a Go-native widget toolkit; the Wails v3 webview UI under `apps/engine-native/cmd/archscope-profiler-app/frontend/` is the single active UI. The former browser React app is archived. |
