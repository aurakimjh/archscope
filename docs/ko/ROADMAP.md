...
- 현재 구현 상태: `profile import`는 pprof, async-profiler, py-spy/rbspy,
  speedscope/dotnet-trace, perf collapsed, JFR JSON stack, StackProf,
  PHP/Xdebug, Swift/async stack, Pyroscope/Parca 계열 snapshot을
  `profile_evidence`로 낸다.
...
- 현재 구현 상태: `stitch analyze`는 기존 result JSON 파일에서 exact/advanced
  match group, timestamp-window/service-alias correlation, trace-profile
  linkage, drilldown row, gap finding, Incident Timeline row, Golden Signal,
  stitched Service Flow dependency를 담은 `stitched_evidence`를 생성한다.
...
- 현재 구현 상태: Pyroscope/Phlare 및 Parca 계열 snapshot은
  `profile_evidence`를 통해 처리하며, OTLP Profiles 승격 기준은
  `OTLP_PROFILES_DECISION.md`에 기록한다.
...
- 현재 구현 상태: `api-contract analyze`는 OpenAPI/AsyncAPI spec과
  access-log/broker result JSON에서 `api_contract_analysis`를 생성하고,
  undocumented, unused, slow, high-error finding을 낸다.
...
- 현재 구현 상태: `architecture-docs draft`는 기존 evidence result에서 arc42
  section draft, context/runtime/deployment row, quality requirement, risk,
  ADR draft row를 담은 `architecture_docs`를 생성한다.
...
- Chrome DevTools `.json` profile import와 local Lighthouse report JSON
  import를 구현했다. Lighthouse는 `browser import`를 통해 report score를
  그대로 보존하고 Core Web Vitals, audit, resource를 bounded/redacted
  `browser_audit_evidence`로 투영한다. Synthetic check export는 계획으로 남긴다.
