...
Implementation status: 현재 Go/Wails 라인에서는 이 trigger가 충족됐다.
`access_log`, `otel_logs`, `trace_import`, `jfr_recording`, `gc_log`,
`thread_dump*`, `database_slow_query`, `broker_log`, `profile_evidence`,
platform evidence가 time-bearing 또는 correlation-key evidence로 참여할 수
있다. 현재 cross-source output은 `incident_timeline`과 `stitched_evidence`다.
...
Timeline correlation은 meta-analyzer pattern이다. Raw source log가 아니라
normalized `AnalysisResult` JSON file 또는 in-memory typed result를 소비한다.
현재 제품에서는 Wails Incident Timeline projection과 Go `stitch analyze`
command가 이 pattern을 구현한다.
...
- `database_slow_query`
- `broker_log`
- timestamp, trace, service, runtime label이 있는 `profile_evidence`
- `kubernetes_evidence` 및 cloud/platform evidence
...
현재 parser 상태: Go `otel analyze` 경로는 JSONL/NDJSON record와 OTLP Logs
JSON `resourceLogs` envelope를 받으며 bounded preview, resource attribute,
instrumentation scope metadata, trace/span reference를 보존한다.
...
- OTel logs: streaming JSONL/NDJSON ingestion, bounded OTLP envelope 처리,
  top-N aggregation, row/table limit를 유지한다.
...
Incident Timeline과 stitched-evidence view는 source family별 lane을 가진
ECharts custom 또는 timeline-like chart로 렌더링한다.

- access log requests/errors
- OTel logs/traces
- JFR runtime events
- profiler/thread/GC evidence
- database, broker, platform, stitched-evidence row
