...
18. [브라우저 지원 매트릭스 (T-209)](#브라우저-지원-매트릭스-t-209)
...
ArchScope는 localhost 웹 앱입니다. T-207/T-208 이후 React UI를
FastAPI에서 서빙하므로, UI를 띄우는 브라우저도 T-206 결정 사항
(`POST /api/files/select`, `WebSocket /ws/progress`, drag-drop +
`File.path` 폴백, ECharts + D3 렌더링, `html-to-image` /
`canvas.toDataURL`을 통한 이미지 내보내기)을 지원해야 합니다. 이
섹션은 릴리스마다 사인 오프하는 **smoke 매트릭스**입니다; `☐` 표시는
해당 브라우저에서 아직 검증되지 않았음을 의미합니다.

### 테스트 셋업

1. wheel 설치: `pip install archscope`
   (또는 소스에서 빌드: `./scripts/build-archscope-wheel.sh`).
2. 서버 시작: `archscope serve`. 기본 브라우저에서
   `http://127.0.0.1:8765`를 자동으로 엽니다.
3. 아래 수동 체크리스트를 각 대상 브라우저에서 반복합니다.
   `archscope serve --no-browser`로 시작하면 OS 기본값 대신 특정
   브라우저에 URL을 직접 붙여넣을 수 있습니다.
4. 결과는 `☐ Not tested`를 `✅ OK`, `⚠️ <caveat>`,
   `❌ <bug + linked issue>` 중 하나로 바꿔서 기록합니다.

분석기 end-to-end 체크에 쓸 fixture는 `examples/profiler/`,
`examples/access-log/`, `examples/thread-dump/` 아래에 있습니다. 적당히
작은 파일이면 어떤 것이든 됩니다.
...
이전 버전은 적극적으로 테스트하지 않습니다. 하한은 ECharts 6 + Vite 5
ESM 번들이 강제합니다.

### 검증 매트릭스

각 셀은 fresh 프로파일/세션에서의 결과를 기록합니다. `☐ Not tested`이면
해당 브라우저에서 지원을 주장할 수 없으니 누군가 채워야 합니다.

| 검증 항목 | Chrome | Edge | Firefox | Safari |
|---|---|---|---|---|
| 정적 UI 로딩 (`/`, 콘솔 에러 없음) | ☐ Not tested | ☐ Not tested | ☐ Not tested | ☐ Not tested |
| 파일 선택 (`Pick file` → 서버 측 절대 경로) | ☐ Not tested | ☐ Not tested | ☐ Not tested | ☐ Not tested |
| 드래그 앤 드롭 | ☐ Not tested | ☐ Not tested | ☐ Not tested | ☐ Not tested |
| Multipart 업로드 폴백 (`/api/upload`) | ☐ Not tested | ☐ Not tested | ☐ Not tested | ☐ Not tested |
| 분석기 end-to-end (Profiler collapsed) | ☐ Not tested | ☐ Not tested | ☐ Not tested | ☐ Not tested |
| 탭 전환 (Summary / Flamegraph / Charts / Drill-down / Diagnostics) | ☐ Not tested | ☐ Not tested | ☐ Not tested | ☐ Not tested |
| WebSocket `/ws/progress` 연결 + `ready` 수신 | ☐ Not tested | ☐ Not tested | ☐ Not tested | ☐ Not tested |
| Cancel 버튼이 `analyze:cancelled` 시그널 | ☐ Not tested | ☐ Not tested | ☐ Not tested | ☐ Not tested |
| ECharts 패널 렌더 (Access Log 요청률) | ☐ Not tested | ☐ Not tested | ☐ Not tested | ☐ Not tested |
| D3 SVG flamegraph (작은 프로파일) | ☐ Not tested | ☐ Not tested | ☐ Not tested | ☐ Not tested |
| Canvas flamegraph 자동 전환 (≥4 000 노드) | ☐ Not tested | ☐ Not tested | ☐ Not tested | ☐ Not tested |
| D3 timeline + bar 차트 | ☐ Not tested | ☐ Not tested | ☐ Not tested | ☐ Not tested |
| 이미지 저장 PNG 1× / 2× / 3× | ☐ Not tested | ☐ Not tested | ☐ Not tested | ☐ Not tested |
| 이미지 저장 JPEG 2× | ☐ Not tested | ☐ Not tested | ☐ Not tested | ☐ Not tested |
| 이미지 저장 SVG (벡터) | ☐ Not tested | ☐ Not tested | ☐ Not tested | ☐ Not tested |
| "모든 차트 저장" 일괄 내보내기 | ☐ Not tested | ☐ Not tested | ☐ Not tested | ☐ Not tested |
| pprof `.pb.gz` 다운로드 | ☐ Not tested | ☐ Not tested | ☐ Not tested | ☐ Not tested |
| HTML 보고서 다운로드 (Export Center) | ☐ Not tested | ☐ Not tested | ☐ Not tested | ☐ Not tested |
| PPTX 보고서 다운로드 (Export Center) | ☐ Not tested | ☐ Not tested | ☐ Not tested | ☐ Not tested |
| 로케일 전환 en ↔ ko | ☐ Not tested | ☐ Not tested | ☐ Not tested | ☐ Not tested |
| 테마 토글 light / dark / system | ☐ Not tested | ☐ Not tested | ☐ Not tested | ☐ Not tested |
| 사이드바 접기 + 영속화 | ☐ Not tested | ☐ Not tested | ☐ Not tested | ☐ Not tested |
| 키보드 단축키 (`/` 검색 등) | ☐ Not tested | ☐ Not tested | ☐ Not tested | ☐ Not tested |
| Settings 페이지 reload 후 영속 | ☐ Not tested | ☐ Not tested | ☐ Not tested | ☐ Not tested |
| Demo Data Center에서 시나리오 실행 | ☐ Not tested | ☐ Not tested | ☐ Not tested | ☐ Not tested |

### 알려진 브라우저 caveat

이 섹션은 버그가 발생할 때마다 채워집니다. 각 항목은 issue 링크,
영향받는 브라우저/버전, workaround(있을 경우), fix-or-defer 결정을
포함해야 합니다.
...
매트릭스는 `docs/en/USER_GUIDE.md`와 `docs/ko/USER_GUIDE.md`에
존재합니다. 행이 통과하면:

1. 셀을 `✅ OK`와 테스트한 버전 (예: `✅ Chrome 134`)으로 편집.
2. 기저 버그를 고친 PR과 같은 PR로 doc 편집 커밋, 또는 코드 변경이
   없으면 별도 documentation 커밋.

행이 실패하면:

1. 이슈를 등록하고 셀을 `❌ #<issue> — 짧은 사유`로 편집.
2. 위의 "알려진 브라우저 caveat" 섹션에 workaround 추가.

`⚠️` 행 (caveat 기록됨)은 릴리스가 가능하지만, Chrome / Edge / Safari의
`❌` 행은 릴리스를 막습니다. Firefox `❌`는 문서화는 하지만
릴리스를 막지는 않습니다 — 프로젝트의 tier-1 플랫폼은 macOS 기본
Safari와 cross-platform Chromium 패밀리입니다.
