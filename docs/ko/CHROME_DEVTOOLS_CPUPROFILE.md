...
**두 수집원은 산출 포맷이 다르다.** Chrome Performance 패널의 저장 기능은
현재 **Save trace**이고 결과물은 `.json` 또는 `.json.gz`이며 Chrome 142부터
gzip이 기본이다. `.cpuprofile`은 Node.js `--cpu-prof`와 CDP `Profiler.stop`의
산출물이다. "Chrome DevTools에서 저장한 파일을 연다"는 사용자 서사가 trace
어댑터를 요구하므로, **3.0에서 trace를 1차 입력으로 확정했다.** 두 경로는
같은 V8 CpuProfile 정규화 코어로 합류한다.

사용자 워크플로는 세 단계로 끝나야 한다.

1. DevTools Performance 패널에서 **Save trace**로 `.json`/`.json.gz` 확보.
   Node.js/CDP 사용자는 `--cpu-prof` 또는 `Profiler.stop`으로 `.cpuprofile`을
   써도 된다 — 둘 다 Phase 1 지원 대상이다(3.0.3).
2. `archscope-engine profile import --in ./trace.json.gz` 또는 데스크톱
   앱에 드래그 앤 드롭(데스크톱 진입점은 Browser 페이지 —
   `AnalyzeProfileEvidence` 경유, 5.0.1/7절).
...
### 3.0 1차 대상 결정 (G1 / T-558) — 확정: Chrome trace
...
`*.json.gz`로 관리된다. 즉 **초안의 Phase 순서로는 Chrome 사용자가 저장한 파일을
열 수 없다.**

> **결정 (2026-07-20): 1차 지원 입력은 Chrome Performance trace(`.json` /
> `.json.gz`)다.** `.cpuprofile`은 별도 Phase가 아니라 **같은 Phase 1에서 함께
> 지원**한다.

#### 3.0.1 근거

`.cpuprofile`을 1차로 두는 안(초안 B)을 기각한 이유는 하나다. **제품 동기를
달성하지 못한다.**

1절이 이 기능의 이유로 든 것은 "프론트엔드 병목의 근거 확보"와 "수집 비용이
사실상 0"이다. 두 근거 모두 **Chrome 사용자가 DevTools에서 저장한 파일을 그대로
열 수 있을 때만** 성립한다. Performance 패널이 `.cpuprofile`을 저장하지 않는
이상, B안의 산출물은 "브라우저 성능 분석"이 아니라 **Node.js/CDP 프로파일
지원**이며, 이는 애초에 착수 근거로 제시된 것이 아니다. 수집 안내가
"먼저 CDP로 프로파일을 뽑으세요"가 되는 순간 "수집 비용 0"이라는 두 번째 근거도
사라진다.

즉 B는 비용이 낮은 안이 아니라 **다른 기능을 만드는 안**이다. 비용 비교의
대상이 아니다.

#### 3.0.2 A안의 비용을 어떻게 낮추는가

A안의 대가로 지목된 것은 "gzip 스트리밍과 대형 파일 처리를 Phase 1에서
떠안는다"였다. **Phase 1의 trace 처리 범위를 좁혀 이 비용의 대부분을 제거한다.**

핵심은 3.2가 이미 말한 구조다 — trace는 `Profile`/`ProfileChunk` 이벤트를
누적 병합하면 3.1과 동일한 구조가 되고, 3.1의 정규화 함수는 **어느 안을 택하든
공통 코어**다. 따라서 Phase 1의 trace 어댑터는 완전한 트레이스 파서가 아니라
**필터**로 정의한다.

| Phase 1 trace 어댑터가 하는 일 | 하지 않는 일 |
|---|---|
| gzip 감지·해제 (스트리밍) | 트레이스 전체를 메모리 모델로 적재 |
| top-level 배열과 `{traceEvents, metadata}` 양쪽 수용 | 임의 trace event의 일반 표현 |
| `ph:"P"`의 `Profile`/`ProfileChunk`만 추출해 id별 누적 | `ph:"X"` 지속 이벤트 해석 (Long Task·Layout·Paint) |
| 병합 결과를 3.1 정규화 함수에 전달 | 이벤트 간 상관·중첩 구조 복원 |
| `metadata`에서 Chrome 버전·프로세스 정보 추출 | 다중 프로세스/스레드 프로파일 병합 (아래) |
...
#### 3.0.3 확정된 Phase 1 입력 계약

| 입력 | 확장자 | Phase 1 | 비고 |
|---|---|---|---|
| Chrome Performance trace | `.json` | 지원 | top-level 배열 / `{traceEvents, metadata}` 양쪽 |
| Chrome Performance trace (압축) | `.json.gz` | 지원 | Chrome 142+ 기본 |
| V8 CPU profile | `.cpuprofile` | 지원 | Node `--cpu-prof`, CDP `Profiler.stop` |
| V8 CPU profile (압축) | `.cpuprofile.gz` | 지원 | 같은 해제 경로 재사용 |

**세 입력이 같은 Phase에 들어가는 것이 이 결정의 요점이다.** 정규화 코어가
하나이므로 `.cpuprofile` 지원은 trace 지원의 부산물이며, 별도 비용이 거의 없다.
1절이 근거로 든 "Node.js와의 대칭"도 이 경로에서 자동으로 얻는다.

다중 프로세스/스레드는 Phase 1에서 **가장 큰 하나를 선택**하고 나머지는
diagnostic으로 알린다. 렌더러가 여러 개인 트레이스에서 조용히 첫 번째를 고르면
잘못된 페이지를 분석하게 된다. 선택 UI는 Phase 2로 둔다.

#### 3.0.4 이 결정에 종속된 것들

0절이 "게이트가 열리기 전까지 확정하지 않는다"고 유보한 항목을 여기서 푼다.

| 항목 | 확정 내용 |
|---|---|
| 문서 제목 | 현행 유지 — "Chrome DevTools / V8 CPU 프로파일"이 두 입력을 모두 포괄한다 |
| 메뉴 문구 | **"브라우저 성능 분석"**을 사용한다. trace가 1차 입력이므로 근거가 성립한다 (7절) |
| 수집 안내 | DevTools Performance 패널 → **Save trace** 를 1차 경로로 안내. `.cpuprofile`은 Node.js/CDP 사용자를 위한 보조 경로로 병기 |
| 지원 매트릭스 | `planned` → Phase 1 대상으로 승격. 포맷명은 `chrome-trace-json`과 `v8-cpuprofile` 둘로 등재 |
| Phase 순서 | 9절 Phase 1이 trace 어댑터를 포함하도록 개정. 초안의 Phase 4 "trace 지원" 항목은 **`ph:"X"` 구간 이벤트 해석**만 남는다 |
...
### 3.2 Chrome Trace Event JSON (`.json`) — 1차 대상 (3.0 확정)
...
초안이 이 포맷을 Phase 4로 미룬 이유는 셋이었다 — 파일이 통상 10~100배 크고,
...
#### 4.3.1 측정 단위·시간 귀속 계약 (G2 / T-559) — 확정
...
##### 결정 1 — 값의 단위는 마이크로초 `int64`다

> **`Sample.Value`는 그 샘플에 귀속된 CPU 시간을 마이크로초 정수로 담는다.**

나노초를 쓰지 않는 이유는 **원본에 없는 정밀도이기 때문**이다. V8의
`startTime`·`endTime`·`timeDeltas`가 전부 마이크로초이므로, 나노초로 올리면
항상 `000`으로 끝나는 값을 저장하며 **정밀도를 얻은 듯한 착시**만 만든다. 반대로
밀리초 `float64`로 내리면 누적 합산에서 오차가 쌓여 아래 불변식의 허용 오차를
정의할 수 없게 된다.

`int64` 마이크로초는 약 29만 년 범위라 오버플로 우려가 없고, **원본과 1:1
대응하므로 왕복 변환이 항등**이다. 이것이 골든 테스트를 쓸 수 있게 만드는 성질이다.

| 층 | 단위 | 타입 |
|---|---|---|
| 원본 V8 | 마이크로초 | JSON number |
| 파서 → `Sample.Value` | 마이크로초 | `int64` |
| 분석기 내부 누적 | 마이크로초 | `int64` |
| 결과 스키마·UI 표시 | 밀리초 | `float64` (표시 직전 1회 환산) |

**환산은 표시 직전 단 한 번이다.** 중간 계층에서 ms로 내렸다가 다시 올리는 경로를
만들지 않는다.

##### 결정 2 — `samples[]`가 정본, `hitCount`는 검증용

> **표본 수는 `samples[]`의 길이에서 나온다. `hitCount`는 교차 검증에만 쓴다.**

3.1이 지적했듯 둘은 중복 정보이고, `hitCount`만으로는 시간축을 복원할 수 없다.
따라서 정본을 `samples[]`로 두는 것이 유일하게 일관된 선택이다. `hitCount`는
버리지 않고 **불변식 검사에 쓴다**(아래 `INV-C3`) — 파서 버그를 잡는 공짜 검증
수단이다.

`samples[]`가 아예 없는 입력(CDP 계약상 유효, 3.1의 optional 필드 정책)은
**집계 전용 경로**로 받는다.

| 입력 | 표본 수 | 시간축 | `duration_ms` |
|---|---|---|---|
| `samples[]` 있음 | `len(samples)` | 사용 가능 | 귀속 시간 합 |
| `samples[]` 없고 `hitCount`만 | `Σ hitCount` | **비활성** + diagnostic | **미제공** — 추정하지 않는다 |

`hitCount`만 있을 때 `duration_ms`를 평균 간격으로 추정하지 않는 것이 요점이다.
그 추정이 바로 초안이 빠졌던 이중 환산의 원인이고, **없는 정보를 만들어내는
것**이다. 표본 수와 비율만 제공하고 시간은 제공하지 않는다.

##### 결정 3 — `IntervalMS` 역산 경로를 V8에서 끊는다

4.3.1 서두가 지적한 이중 환산은 **`IntervalMS`에 값을 넣는 방식으로는 해결되지
않는다.** `0.001`을 넣는 "최소 변경"안은 `total_samples`·`sample_ratio`가 사실상
마이크로초를 뜻하게 만들어 계약 부채를 남긴다. 따라서 **권장안을 채택한다.**

- 파서가 `Parsed.ValueUnit = "microseconds"`를 채운다. 이 값은
  `parser_metadata` 안이 아니라 **`Parsed`의 1급 필드**여야 공통 분석기가 읽는다.
- 공통 분석기는 `ValueUnit`을 보고 분기한다.
  - `"samples"` (기존 22종 포맷): 현행 `IntervalMS` 역산 경로 유지 — **회귀 없음**
  - `"microseconds"` (V8 경로): 역산을 **건너뛰고** `Σ Value`를 직접 duration으로 쓴다
- `unified_profile_schema.sample_unit`의 `"samples"` 하드코딩(`analyzer.go:78`)을
  `ValueUnit` 파생값으로 전환한다.
- 시간 환산을 **단일 함수**에 모은다. 현재 `analyzer.go:313-314`에 인라인된
  계산이 유일한 환산 지점이 되어야 한다.

기존 포맷이 `"samples"` 분기를 그대로 타므로 **이 변경은 22종 포맷에 대해
동작이 동일하다.** 수정 범위가 넓다는 대가는 실제로는 "분기 하나 + 하드코딩 하나
제거"이며, 초안이 우려한 것보다 작다.

##### 결정 4 — 시간 귀속 규칙

`timeDeltas[0]`은 공식 명세상 `startTime`에서 첫 샘플까지의 간격이다. 초안의
"첫 샘플에 두 번째 delta나 중앙값을 쓴다"는 규칙은 **근거가 없고 총시간을
왜곡한다.** timestamp 복원과 비용 귀속을 분리한다.
...
| 지표 | 정의 |
|---|---|
| `recording_duration_us` | `endTime - startTime`. **표시 옵션과 무관한 상수** |
| `active_duration_us` | `(idle)` 제외 샘플의 귀속 시간 합 |
...
**골든 테스트로 먼저 고정한 뒤 구현한다.** 허용 오차 ε는 마이크로초 정수 연산
기준 **0**이며, 클램프가 발생한 입력에서만 클램프량만큼 허용한다.

| ID | 불변식 |
|---|---|
| `INV-C1` | `sampled_duration_us == active_duration_us + idle_duration_us` |
| `INV-C2` | `sampled_duration_us == endTime - startTime` (클램프량 제외) |
| `INV-C3` | 모든 노드에 대해 `count(samples[] 중 그 노드 id) == hitCount` — 불일치 시 diagnostic |
| `INV-C4` | 모든 노드에 대해 **`total_us == self_us + Σ(자식 total_us)`** |
| `INV-C5` | 루트의 `total_us == sampled_duration_us` |
| `INV-C6` | `Σ(전 노드 self_us) == sampled_duration_us` |
| `INV-C7` | CLI 기본 옵션과 Wails 기본 옵션이 동일한 결과를 산출 |
| `INV-C8` | Diff 정규화 여부와 무관하게 baseline/target 단위가 동일 |
| `INV-C9` | `--include-idle` 전후로 `recording_duration_us`가 불변 |

`INV-C4`가 이 절이 요구받은 핵심 불변식이며, `INV-C5`·`INV-C6`은 그것이 트리
전체에서 성립함을 다른 각도에서 검사한다. 셋을 함께 두는 이유는 **`INV-C4`만으로는
누락된 서브트리를 못 잡기 때문**이다 — 어떤 노드가 통째로 빠져도 남은 노드들끼리는
`total = self + children`이 성립한다. `INV-C6`이 그 누락을 잡는다.

`self_us`와 `total_us`의 정의를 명시한다. **`self_us`는 그 노드가 `samples[]`의
리프로 등장한 샘플들의 귀속 시간 합**이고, `total_us`는 그 노드를 스택에 포함한
모든 샘플의 귀속 시간 합이다. 재귀 호출로 같은 노드가 한 스택에 여러 번 나타나도
`total_us`에는 **한 번만** 계상한다 — 그러지 않으면 `INV-C5`가 재귀 함수가 있는
모든 프로파일에서 깨진다.
...
### 5.0 데스크톱 단일 분석 경로 (G3 / T-560) — 확정: A안 경로 통합, 단계적 이행
...
- 현재 `AnalyzeProfileEvidence`를 호출하는 프론트엔드 페이지는 **없다**
  (frontend 소스 전체에서 호출 0건). 즉 신규 경로는 CLI 전용으로만 존재한다.

즉 **"CLI import 성공"과 "데스크톱/Diff 성공"은 서로 다른 구현 과제다.**

#### 5.0.1 결정 — A안(경로 통합)을 채택한다

**2026-07-20 확정.** B안(병행)은 기각한다. B안의 "Diff에만 공통 로더 연결"은
결국 A안의 1단계와 같은 작업을 하면서 종착점만 이원화로 고정하는 안이다.
이원화가 고착되면 신규 포맷마다 "legacy에도 추가하나?"라는 같은 질문이
반복되고, T-562(분류기 이원화)와 같은 종류의 분기가 서비스 계층에서 하나 더
생긴다. 대가로 지목됐던 "기존 프로파일러 페이지 회귀 위험"은 아래 이행 순서로
격리한다 — legacy 화면을 한 번에 갈아끼우는 단계가 없다.

**정규화 계약.** 유일한 정규화 지점은
`internal/parsers/profile.Parsed`(`[]Sample`)이며, 유일한 데스크톱 분석 정본
경로는 `EngineService.AnalyzeProfileEvidence` →
`internal/analyzers/profile.Build` → `profile_evidence` `AnalysisResult`다.
Phase 1의 `chrome-trace-json`/`v8-cpuprofile` 파서는 `parsers/profile`에
추가되는 순간 CLI·데스크톱·Diff·Export가 **같은 코드로** 이를 얻는다. 신규
포맷을 legacy `ProfilerService`의 포맷 스위치
(`runAnalyze`/`loadStacks`/`detectFormat`)에 추가하는 것은 **금지한다** — 그
스위치는 동결(frozen)이며 축소 대상이다.

**이행 순서.** 각 단계는 독립적으로 릴리스 가능하고, 앞 단계가 뒷 단계를
막지 않는다.

| 단계 | 내용 | 회귀 노출면 |
|---|---|---|
| 1. Diff 공통 로더 | Go에 `stacksFromParsed(parsed parser.Parsed) map[string]int` 로더를 추가하고 `ProfilerService.loadStacks`가 4종 스위치 실패 시(또는 우선적으로) 이 로더에 위임. `DiffPage.tsx`의 `DiffFormat` 4종 고정을 auto-detect 위임으로 대체 | Diff 페이지만. 기존 4종은 기존 경로 결과와 diff 골든 테스트로 동등성 보증 |
| 2. Browser 페이지 신설 | `BrowserCpuProfilePage`(Phase 4 항목 18)는 처음부터 `AnalyzeProfileEvidence`만 호출. legacy 서비스 의존 없음 | 신규 화면뿐, 기존 화면 무변경 |
| 3. Workspace/Export 접속 | Browser 페이지의 성공 결과를 Analysis Workspace 세션 레지스트리에 `profile_evidence`로 등록(T-425 계약). Export Center(T-424)와 report pack(T-460)은 등록된 결과를 기존 소비 경로로 처리 — **신규 export 코드 없음** | Workspace 등록 한 줄. Export 파이프라인 무변경이 검증 대상 |
| 4. 기존 프로파일러 페이지 전환(후속) | `ProfilerAnalyzerPage`의 collapsed 입력을 `AnalyzeProfileEvidence` 경유로 전환하고 legacy 서비스는 Jennifer/SVG/HTML 전용 어댑터로 축소. 1~3단계 안정화 이후 별도 커밋 | 기존 화면 전체. 가장 위험하므로 마지막이고, T-560 완료 조건에 포함하지 않는다 |

**7.6과의 정합.** A안 최종 상태에서 기존 `profiler` 메뉴도 auto-detect 전
포맷을 받게 되지만, 그 시점은 4단계 이후다. 1~3단계 동안 `.cpuprofile`/trace의
데스크톱 창구는 Browser 페이지뿐이며, 이는 7.6의 "전용 창구" 방침과 일치한다.

**수용 기준.**

- **AC-1 (CLI/데스크톱 패리티)**: 같은 파일에 대해 CLI `profile import`와
  `AnalyzeProfileEvidence`가 타임스탬프성 필드를 제외하고 동일한
  `AnalysisResult`를 낸다. 4.3.1의 INV-C9와 같은 골든 fixture로 고정한다.
- **AC-2 (Diff 전 포맷)**: `chrome-trace-json` 두 파일의 Diff가 legacy Diff
  UI에서 동작하고, 기존 4종의 Diff 결과는 로더 교체 전후 동일하다.
- **AC-3 (Export 무변경 검증)**: Browser 페이지에서 분석한 결과가 Workspace에
  나타나고 Export Center에서 JSON/HTML export가 성공한다. 이 흐름이 통과해야
  "Report Export 변경 없음"을 주장할 수 있다.
- **AC-4 (legacy 동결)**: `profilerservice.go`의 포맷 스위치에 신규 포맷이
  추가되지 않았음을 리뷰에서 확인한다(기계적 가드는 두지 않는다).

이 결정으로 **G3는 닫히고 Phase 4(데스크톱 통합)의 착수 조건이 갖춰진다.**
구현 순서는 위 이행 표를 따르고, 4단계는 T-560의 범위 밖 후속 작업이다.
...
> **G1 해제 (2026-07-20).** 메뉴 이름은 **"브라우저 성능 분석"**, 수집 안내는
> DevTools **Save trace** 1차 + `.cpuprofile` 병기, 파일 필터는
> `.json`/`.json.gz`/`.cpuprofile`/`.cpuprofile.gz`로 확정했다(3.0.4).
> **G3 해제 (2026-07-20).** A안 경로 통합이 확정되어(5.0.1) 신규 페이지는
> `AnalyzeProfileEvidence`만 호출한다. 7.6의 "엔진 공유" 서술은 이행 1~3단계가
> 끝나면 Browser 페이지에 대해 성립하고, 기존 `profiler` 메뉴에 대해서는
> 이행 4단계(후속) 이후 성립한다.
...
`profiler` 메뉴가 `.cpuprofile`을 받을지는 **5.0.1의 G3 결정으로 확정됐다.**
A안(경로 통합)이 채택되었으므로 최종 상태에서는 기존 메뉴도 auto-detect 전
포맷을 받는다. 다만 그 시점은 이행 4단계(기존 페이지 전환) 이후이며, 1~3단계
동안 `.cpuprofile`/trace의 데스크톱 창구는 Browser 페이지뿐이다. 브라우저
메뉴는 대체가 아니라 **전용 창구**다.
...
메모리 가드는 `internal/profiler`(collapsed 파일 경로)와 V8/Chrome 입력
경로에 각각 있다.
...
V8/Chrome 입력은 `ParseFile`이 전체 `[]byte` 경로보다 먼저 선택한다. gzip을
스트리밍 해제하면서 256 MiB(또는 호출자가 정한 `MaxBytes`)를 강제하며, Chrome
trace는 top-level `traceEvents` 배열을 `json.Decoder`로 한 이벤트씩 순회한다.
`ph:"P"`가 아닌 이벤트는 보관하지 않는다.

직접 `.cpuprofile`은 decoder로 읽으므로 원본 전체 바이트와 별도 RawMessage
envelope를 중복 보관하지 않는다. 다만 그래프 검증과 `samples`/`timeDeltas`
대응을 위해 node map과 정수 배열은 유지한다. 따라서 다중 GB 구조화 입력의
무제한 지원을 주장하지 않으며, 크기 상한은 계속 안전 경계다.
...
**(a-1) T-564 구현 범위.** V8/Chrome은 전체 document `ReadAll` 및 trace
`[]RawMessage` 이중 unmarshal을 쓰지 않는다. `auto` detect도 64 KiB preview만
확인해 V8/trace 전용 경로를 고른다. 다른 구조화 프로파일 포맷은 별도 작업까지
기존의 보수적 크기 가드 경로를 유지한다.

포맷별로 다음을 구분해 설계한다.

| 대상 | 전략 |
|---|---|
| `.cpuprofile` | `os.Stat` / gzip sniff → 제한된 decoder 경로 → graph 검증 및 bounded output aggregation |
| Performance trace | gzip 스트리밍 해제, top-level array와 `{traceEvents:[]}` 양쪽 지원, `Profile`/`ProfileChunk`만 선택 추출 |
| detect | 전체 unmarshal 대신 64 KiB preview로 시그니처 판정 |

**(b) 부분 결과 계약.** 크기 한계를 넘거나 JSON/그래프가 malformed이면 결과를
반환하지 않는 error다. 반대로 샘플 상한에 따른 축약은 성공 결과이며
`parser_metadata.partial_result=true`, 원본/출력 sample 수, downsampling 방법을
기록하고 `PROFILE_DOWNSAMPLED` warning을 낸다. Browser CPU 화면은 이 상태에서
"Large-profile partial result"와 두 sample 수를 표시한다.
...
bucket/weighted aggregation**을 사용한다. 시간 순서의 결정적 bucket마다 첫
stack/timestamp를 anchor로 두고 모든 `Value`(microsecond delta)를 합산한다.
결과는 top-level 병목과 총 sampled duration을 보존하지만 개별 sample-run 세부는
집계됐음을 UI가 알려야 한다.
...
**(f) 스트리밍(Phase 1 완료).** 3.2 trace는 `json.Decoder`로 `traceEvents`
배열을 토큰 단위 순회하며 `Profile`/`ProfileChunk`의 `ph:"P"` 데이터만
사용한다. gzip trace, object wrapper와 bare array wrapper를 모두 지원한다. 샘플
제한이 필요한 trace는 첫 번째 순회에서 개수만 세고, 두 번째 순회에서 선택된
weighted bucket만 만든다. 이 경로는 큰 trace document를 `[]json.RawMessage`로
만들지 않는다.
...
어댑터 나중**이다 — 코어가 두 입력의 공통 기반이므로 먼저 안정화해야 한다.

1. `cpuprofile.go`에 `parseV8CpuProfile` 구현 (4.3) — **정규화 코어**
2. 4.3.1의 단위 계약 구현: `Parsed.ValueUnit` 1급 필드, 공통 분석기의
   `ValueUnit` 분기, `sample_unit` 하드코딩 제거, 시간 환산 단일 함수화
3. `INV-C1`~`INV-C9` 골든 테스트를 **구현보다 먼저** 고정 (4.3.1)
4. `chrome_trace.go`에 `ph:"P"` 필터 어댑터 구현 (3.0.2) — gzip 스트리밍 해제,
   array/object wrapper 양쪽 수용, `Profile`/`ProfileChunk` id별 누적 병합
5. 다중 프로세스/스레드 시 최대 프로파일 선택 + diagnostic (3.0.3)
6. `detect()` / `canonical()` 확장 + 기존 3개 fixture detect 회귀 테스트 (4.2).
   포맷명은 `chrome-trace-json`과 `v8-cpuprofile` 둘로 등재
7. fixture 추가 (아래 목록)
8. `cpuprofile_test.go` — 스택 복원, 음수 delta, id 비연속, `(idle)` 제외,
   빈 `functionName`, 4.6의 그래프 검증 케이스 전부
9. `cmd_profile.go:41` `--format` 도움말 갱신

**2·3번이 1번과 같은 Phase에 있는 것이 중요하다.** 단위 계약을 나중에 바꾸면
그때까지 만든 fixture의 기대값을 전부 다시 만들어야 한다.
...
- 공식 Chrome Performance trace JSON, JSON.GZ, object/array wrapper (**G1 확정에
  따라 필수**)
- 다중 렌더러 프로세스 trace — 최대 프로파일 선택과 diagnostic 확인 (3.0.3)
- Node.js `--cpu-prof` fixture, CDP direct profile fixture
- **같은 세션에서 뽑은 trace와 `.cpuprofile` 쌍** — 두 경로가 동일한 정규화
  결과를 내는지 (3.0.3의 공통 코어 전제 검증)
...
- **재귀 호출이 포함된 프로파일** — `INV-C5`의 중복 계상 회귀 (4.3.1)
- `hitCount`와 `samples[]` 집계가 불일치하는 손상 입력 (`INV-C3`)
- **CLI와 Wails 결과의 단위 / summary / flamegraph parity** (`INV-C7`)
...
23. Browser 페이지 결과의 Analysis Workspace 등록 + Export Center 소비 검증
    (5.0.1 이행 3단계, AC-3) — 항목 18 의존

**완료 기준**: 256MB 초과 파일이 OOM 없이 명확한 오류 또는 다운샘플 결과를 내고,
브라우저 담당자가 전용 메뉴에서 수집 안내를 보며 **Chrome에서 저장한 trace**를
열 수 있다.

> 항목 21이 바뀐 이유는 3.0 확정 때문이다. 트레이스 **스트리밍 파서**는 Phase 1로
> 올라갔고, Phase 4에 남는 것은 초안이 진짜 어려운 부분으로 지목했던
> **구간 이벤트 모델링**뿐이다.
