...
| G3 데스크톱 경로 | 신규 importer와 현재 `ProfilerService`/`DiffPage`가 서로 다른 서비스 경로를 사용(5.0, 7.6) | T-560 | 미결 |
| G4 시간축 의미 | CPU 샘플만으로는 브라우저 Long Task 경계를 복원할 수 없음(9 Phase 3) | T-561 | 미결 |

G1·G2가 닫히면서 **Phase 1(파싱)과 Phase 2(분류)의 착수 조건이 갖춰졌다.**
7절의 메뉴 문구와 수집 안내도 3.0.4에서 확정했다. 남은 G3·G4는 각각 Phase 4
데스크톱 통합과 Phase 3 시간축의 게이트이므로, **그 두 Phase에 한해 착수를
막는다.**
...
2. `archscope-engine profile import --in ./trace.json.gz` 또는 데스크톱
   앱에 드래그 앤 드롭(데스크톱 진입점은 G3 결정에 종속, 7절).
...
> **G3는 여전히 종속**이다 — 7.6의 "엔진 공유" 서술은 5.0의 데스크톱 경로
> 통합이 끝나야 성립한다.
