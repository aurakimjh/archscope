...
2. `archscope-engine profile import --in ./trace.cpuprofile` 또는 데스크톱 앱의
   **브라우저 성능 분석 › CPU 프로파일**에 드래그 앤 드롭.
...
**(c) 롱태스크 하이라이트.** Phase 3의 항목 12가 만드는 50ms 초과 구간을
타임라인에 표시하고 클릭 시 해당 구간의 스택으로 이동한다. **Phase 3 의존
기능이므로 Phase 4의 메뉴 신설 시점에는 비활성 상태로 둔다.** 순서상 메뉴가
먼저 생기고 롱태스크 뷰가 나중에 채워진다.
...
`profiler` 메뉴는 `.cpuprofile`을 **계속 받는다.** auto-detect가 포맷 무관이라
막을 이유가 없고, CLI로 넣은 결과를 열 수도 있다. 브라우저 메뉴는 대체가 아니라
**전용 창구**다.
