...
Phase 2로 미루는 이유: 파일이 통상 10~100배 크고(전체 트레이스), 스트리밍
...
걸리지 않는다. Phase 2에서 `traceEvents` 또는 `ph`/`ProfileChunk` 시그니처로
별도 분기한다.
...
| Timeline | `buildTimeline`(`timeline.go:100`) | **변경 필요** — 아래 |
...
**(f) 스트리밍(Phase 2).** 3.2 트레이스 포맷은 `json.Decoder`로 `traceEvents`
...
FlameGraph와 top stacks를 담은 `profile_evidence`를 산출한다.

### Phase 2 — 분류 및 시간축

6. `runtime_classification_rules.json`에 브라우저 규칙 추가 + 순서 검증 (6.1)
7. `classify.go` 브라우저 토큰 매핑 (6.2)
8. `ts_us` 라벨 기반 실제 타임스탬프 타임라인 (5절)
9. Node.js `--cpu-prof` fixture로 회귀 확인

**완료 기준**: React 앱 프로파일에서 컴포넌트 렌더와 GC가 분리되어 보인다.

### Phase 3 — 견고성

10. `ParseFile` 크기 가드 (8-a) — 전 포맷 공통
11. 다운샘플링 + diagnostic + UI 노출 (8-c)
12. 깊이 절삭 (8-d)
13. 대형 fixture(100MB+) 벤치마크를 `PERFORMANCE.md` 기준에 추가

**완료 기준**: 256MB 초과 파일이 OOM 없이 명확한 오류 또는 다운샘플 결과를 낸다.

### Phase 4 — 데스크톱 UI 및 트레이스

14. `.cpuprofile` 드래그 앤 드롭, i18n, 도움말 (7절)
15. Chrome Trace Event 스트리밍 파서 (3.2, 8-f)
16. 배포 전후 `.cpuprofile` Diff 검증

Phase 1~2가 사용자 가치의 대부분을 만든다. Phase 3은 Phase 1 완료 후 실제 파일
크기를 측정해 우선순위를 재조정한다.
