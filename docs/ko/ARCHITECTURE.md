...
```text
┌──────────┐     ┌──────────┐     ┌──────────────┐     ┌─────────────┐
│ React UI │     │ Preload  │     │ Electron Main│     │ Python CLI  │
└────┬─────┘     └────┬─────┘     └──────┬───────┘     └──────┬──────┘
     │                 │                  │                     │
     │ analyzeAccessLog│                  │                     │
     │ ({filePath,     │                  │                     │
...
```text
Python CLI exit ≠ 0  또는  JSON 파싱 실패
      │
      ▼
Electron Main: BridgeError 생성
  { code: ENGINE_EXITED | ENGINE_OUTPUT_INVALID,
    message: 사용자용 설명,
    detail: stderr 또는 exception trace }
      │
      ▼
IPC response: { ok: false, error: BridgeError }
      │
      ▼
React UI: ErrorPanel 렌더링 (code + message + expandable detail)
```
...
### 확장 예시: 새 Analyzer 추가 흐름

```text
예시: "Redis Slow Log Analyzer" 추가

1. models/redis_slow_log.py
   - RedisSlowLogRecord dataclass 정의
   - fields: timestamp, duration_us, command, key, client_ip

2. parsers/redis_slow_log_parser.py
   - iter_redis_slow_log_records(path) generator 구현
   - tolerant 모드: malformed line skip + diagnostics 수집

3. analyzers/redis_slow_log_analyzer.py
   - streaming aggregation: total commands, avg duration, top slow commands
   - AnalysisResult 생성 (type="redis_slow_log")

4. cli.py
   - @app.command() 추가: archscope-engine redis analyze --file ... --out ...

5. apps/desktop/src/charts/
   - RedisSlowLogDuration template 추가 (chartTemplates.ts)
   - option builder 추가 (chartFactory.ts)

6. apps/desktop/src/pages/RedisAnalyzerPage.tsx
   - FileDropZone + Analyze 버튼 + 결과 렌더링
```
