...
| ETW | 이벤트 귀속 | **미검증** (`Q-WIN-ETW-PAYLOAD`) | header PID를 발신 프로세스로 쓰지 말라는 공식 주의 존재 (`V-WIN-ETW-TCPIP`) |
| WFP | 필터 계층 관측 | **미검증** (`Q-WIN-WFP-ATTR`) | — |
...
| `SEC-8` | CA 개인키 탈취자 | 키 파일 위치·권한 확인, export 기능 탐색 | 키체인/DPAPI 저장. 파일 폴백 시 0600. **export 경로 없음** |
| `SEC-9` | CA 개인키 탈취자 | 두 머신에서 생성한 CA 비교 | 서로 다름. 빌드에 동봉된 CA 없음 |
| `SEC-10` | 크래시 덤프 수집 | 캡처 중 강제 크래시 후 덤프 검색 | CA 개인키·평문 바디 미포함 |
| `SEC-11` | 공용 워크스테이션 타 사용자 | 다른 계정으로 세션 디렉터리 접근 | OS 권한으로 차단 |
| `SEC-12` | — | CA 제거 후 신뢰 저장소 전수 확인 | 모든 저장소에서 제거. 부분 실패 시 명시 보고 (11.2.1) |
...
| 6 | Phase 1 HAR importer | 방언·malformed·security fixture 코퍼스로 정규화·진단 검증 | **코퍼스 구축됨 (2026-07-20, T-572).** `../projects-assets/test-data/har-fixtures/` — 합성 우선 20종 + 생성기, manifest에 기대 diagnostic 고정. importer 검증 실행과 실기 export 보강은 Phase 1 구현과 함께 |
| 7 | deterministic aggregate | 7.4.3의 순서 무관 property test 통과 | 미착수 (T-573) |
| 8 | 프록시 build-vs-integrate spike | 아래 참조 | 미착수 (T-574) |
| 9 | Windows 커버리지 proof | 10.4의 `CAP-1`~`CAP-6` 판정. 실패 시 절대 ratio 제거 | 미착수 (T-571 실측) |
| 10 | Windows Phase 2 캡처 | disk full/crash/event loss/CA failure/pinning/streaming E2E 통과 | 미착수 |
| 11 | HTTP 전용 Diff | URL 템플릿화와 시간 정렬 신뢰도를 포함한 비교가 검증됨 (6.6) | 미착수 (T-575) |
...
| `Q-WIN-ETW-PAYLOAD` | — | — | — | ETW 이벤트 payload의 실제 필드 구성과 고부하 시 손실률 | **미확인.** 10.1.2가 coverage ratio 노출을 이 실측 뒤로 미루는 이유 | open |
| `Q-WIN-WFP-ATTR` | — | — | — | WFP 관측이 프로세스 귀속을 어느 정확도로 제공하는지 | **미확인.** 9.3 fidelity 표의 WFP 행은 이 값이 채워지기 전까지 `미검증` | open |
