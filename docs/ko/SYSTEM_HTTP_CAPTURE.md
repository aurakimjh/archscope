...
> - 2026-07-19 (본 개정): Codex 설계 검토(P0 1~5, P1 6~9, P2 10) 반영. 주요 철회
...
analyzer**가 필요하다. 12.4절과 Phase 4의 항목으로 둔다.
...
| 공용 워크스테이션의 다른 사용자 | 같은 머신의 다른 계정 | 세션 디렉터리·CA 키 열람 | OS 키체인 저장, 0600 폴백, 앱 데이터 경로 권한 (11.2) |
...
| 크래시 덤프·로그 수집 | 프로세스 메모리 스냅샷 | 평문 바디·CA 키 유출 | 키 즉시 폐기, 덤프에서 캡처 버퍼 제외 (11.2) |
...
| 순서 | 게이트 | 완료 조건 |
|---|---|---|
| 1 | 조사 근거 고정 | 모든 `[V]`에 URL/commit/조회일이 있고, 절대 명제에 조사 범위가 명시됨 (부록 A) |
| 2 | Windows capability·fidelity 계약 | 2.1의 tier와 6.2.1의 fidelity 등급이 모드별 표로 고정됨 |
| 3 | 보안 위협 모델 | 11.0의 위협 모델과 11.3의 기본값이 승인됨 |
| 4 | bounded result/store 계약 | 6.6의 봉투·스토어 경계가 승인됨 |
| 5 | Phase 1 HAR importer | 방언·malformed·security fixture 코퍼스로 정규화·진단 검증 |
| 6 | deterministic aggregate | 7.4.3의 순서 무관 property test 통과 |
| 7 | 프록시 build-vs-integrate spike | 아래 참조 |
| 8 | Windows Phase 2 캡처 | disk full/crash/event loss/CA failure/pinning/streaming E2E 통과 |
| 9 | Windows 커버리지 심화 | ETW/WFP/Npcap proof가 있는 scope에서만 coverage ratio 노출 (10.1.2) |
| 10 | HTTP 전용 Diff | URL 템플릿화와 시간 정렬 신뢰도를 포함한 비교가 검증됨 (6.6) |
...
검토 과정에서 확인 경로가 확정된 항목이다. 나머지 `[V]`는 Phase 0에서 채운다.

| 주제 | 출처 |
|---|---|
| mitmproxy local capture 대상 지정 | [Proxy Modes — Local Capture](https://docs.mitmproxy.org/stable/concepts/modes/) |
| Charles client process 도구 | [Client Process Tool](https://www.charlesproxy.com/documentation/tools/client-process/) |
| FiddlerCore 시작 설정 | [FiddlerCoreStartupSettings](https://docs.telerik.com/fiddlercore/api/fiddler.fiddlercorestartupsettings) |
| DevTools HAR export 구현 | [NetworkPanel.ts](https://chromium.googlesource.com/devtools/devtools-frontend/+/HEAD/front_end/panels/network/NetworkPanel.ts) |
| Go `net/http` 헤더·해제 동작 | [pkg.go.dev/net/http](https://pkg.go.dev/net/http) |
| Wails 이벤트 API | [Events](https://wails.io/docs/reference/runtime/events/) |
| Windows 네트워크 성능 카운터 | [net-sub-performance-counters](https://learn.microsoft.com/en-us/windows-server/networking/technologies/network-subsystem/net-sub-performance-counters) |
| `GetExtendedTcpTable` | [iphlpapi](https://learn.microsoft.com/en-us/windows/win32/api/iphlpapi/nf-iphlpapi-getextendedtcptable) |
| Windows TCP/IP ETW 이벤트 | [etw/tcpip](https://learn.microsoft.com/en-us/windows/win32/etw/tcpip) |
| Linux `/proc/<pid>/net` 범위 | [proc_pid_net(5)](https://man7.org/linux/man-pages/man5/proc_pid_net.5.html) |

### A.3 재확인이 필요한 주장

| 주장 | 상태 |
|---|---|
| "mitmproxy는 수집 이후 PID 필드를 보존하지 않는다" | mitmproxy 공식 문서는 local capture가 전체 장치 또는 **process name/PID를 대상으로 삼을 수 있다**고 명시한다. 본문의 주장은 그보다 구체적인 코드 수준 주장일 수 있으므로, **해당 source commit과 검색 경로**를 원장에 남기기 전까지 근거로 쓰지 않는다 |
| 1.5.4절의 두 절대 명제 | 조사 범위 한정 표현으로 하향 조정 완료. 원장에 확인 경로 기재 필요 |
| 경쟁 도구 가격·버전표 | "as of" 표기 추가 및 주기적 갱신 필요 |
