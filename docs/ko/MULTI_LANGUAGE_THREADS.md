...
`AnalysisResult(type="thread_dump_multi")`를 만들고 아래 핵심
finding을 발화합니다.
...
Java/JVM 전용 메타데이터는 언어 비의존 모델을 바꾸지 않고 선택적
finding과 테이블로 추가됩니다.
...
- 보조 테이블: `native_method_threads`,
  `class_histogram_top_classes`. Class histogram은 텍스트
  `num  #instances  #bytes  class name` 블록만 파싱하며 HPROF heap dump는
  파싱하지 않습니다. Parser metadata는 기본 500행까지 보존합니다. 더
  크게 보려면 CLI `--class-histogram-limit N`, HTTP
  `classHistogramLimit`, 또는 환경변수
  `ARCHSCOPE_CLASS_HISTOGRAM_ROW_LIMIT`을 사용합니다.

Java jstack lock은 `lock_entry_wait`, `object_wait`,
`parking_condition_wait`로 wait mode를 분리합니다. 순수 `Object.wait()`
수면은 실제 lock-entry wait 증거가 없으면 lock contention hotspot으로
보고하지 않습니다.
