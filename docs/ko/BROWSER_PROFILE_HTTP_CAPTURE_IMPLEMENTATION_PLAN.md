...
- **Part A — Chrome/V8 프로파일 분석:** 핵심 엔진·데스크톱 UI·Diff·Workspace·
  문서·fixture·릴리스 smoke가 구현 완료됐다. 다만 이번에 도입하는 운영 규칙에
  따른 독립 구현 리뷰는 아직 없으므로 `C-RG1`은 **구현 완료 / 리뷰 대기**다.
- **Part B — HTTP 캡처 및 분석:** Phase 0 설계 게이트와 proxy PoC, 결정적 집계기는
  완료됐다. HAR 파서·분석기·최소 import 화면도 존재하지만, 설계가 요구하는 방언
  코퍼스 골든 연동, 전용 리댁션·자원 제한, 타임라인/브러시, 상세/필터 UI는 아직
  완료되지 않았다. 실시간 Windows 캡처는 T-571 실 NIC 증거가 닫히기 전에는
  시작하지 않는다.
...
| 1 | `C-RG1` Chrome/V8 릴리스 구현 승인 | **리뷰 준비 완료** | 독립 리뷰 `PASS` |
| 2 | `H-RG1` HAR 오프라인 분석 완성 | 대기 | `C-RG1 PASS`; 그룹 내부 `H-SEC1 PASS` |
...
**상태:** 구현 완료, 독립 리뷰 대기. 새 기능을 더 넣는 그룹이 아니라 이미 완료된
T-558~T-565를 새 리뷰 정책으로 승인하는 그룹이다.
...
- [ ] 독립 리뷰에서 발견되는 UI 회귀나 접근성 지적사항 반영
...
- [ ] `CaptureTransaction`/timing state/fidelity 계약을 실제 Go 모델에 반영한다.
- [ ] HAR 파서를 `detect → structural validate → dialect → normalize → model map →
  redact → analyze` 단계로 분리한다.
- [ ] BOM, malformed/deep/oversized JSON, entry/string/body 수 제한과 deterministic
  diagnostic을 구현한다.
- [ ] Chrome/Firefox/Safari/Charles/Fiddler/Proxyman/Insomnia/generic 방언을
  `dialect.go`의 1급 계약으로 분리한다.
- [ ] `../projects-assets/test-data/har-fixtures/` manifest를 읽는 골든 테스트를
  연결하고 20개 합성 fixture의 dialect/diagnostic/redaction assertion을 고정한다.
- [ ] URL뿐 아니라 header/query/cookie/JWT/body/process metadata까지 전용
  `capture/redact` 정책을 적용한다.
- [ ] summary/series/table을 bounded `AnalysisResult`로 유지하고 HAR 상세 행의
  inline 상한과 잘림 diagnostic을 고정한다.
- [ ] CLI·Wails 결과 parity와 실제 Chrome/Firefox export fixture 보강 절차를
  검증한다.
...
- [ ] `(HAR 가져오기)` pseudo-process tree와 요약 카드
- [ ] 전체 타임라인, brush 선택, 선택 구간 재집계
- [ ] 트랜잭션 목록과 request/response/timing/process 상세 탭
- [ ] method/status/host/path/MIME/duration/error/fidelity 필터
- [ ] dialect·fidelity·redaction·parser diagnostic과 degenerate timestamp 안내
- [ ] bounded row 렌더링, 빈 상태, 실패/부분 결과, Workspace 등록 회귀 테스트
...
현재 다음 행동은 `C-RG1` 독립 리뷰 요청이다. 리뷰 범위는 Chrome/V8 구현과
2026-07-21 release smoke이며, 이 리뷰가 `PASS`되기 전에는 `H-RG1` 코드를 시작하지
않는다. 리뷰 지적이 들어오면 Chrome 엔진 수정은 Codex, UI 수정은 Claude가 맡고,
같은 그룹에서 재검증·재리뷰한다.
