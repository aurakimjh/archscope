...
Parser debug logs are generated by the engine as separate developer artifacts. They are not part of the `AnalysisResult` transport contract. In portable desktop runs, Electron passes a debug-log directory under the executable location so field users can send the `archscope-debug/` folder without touching the original log directory. Debug logs are redacted by default and preserve parser evidence through field shapes and partial-match metadata.
