...
the fourth `CONDITIONAL`. The code-side S1–S9 conditions are verified, but V1
blocked the first schema-v4 artifact because 57 Edge/Electron background rows
contradicted its `fixtureTrafficOnly: true` declaration. The harness now filters
published rows to loopback fixtures and derives its privacy fields from the
result. A fresh Windows artifact, checksum, and narrow independent re-review
remain. T-582 stays blocked until an independent `PASS`.
...
- [x] R2 harness mechanism: require an acceptance WebView2 CDP port and a real
  ...
  stores. The first artifact was produced but rejected for V1 privacy drift; it
  must be regenerated by the corrected harness.
...
- [x] V1 harness: run Edge/Electron with ephemeral profiles and background-
  ...
  and local-path absence from the actual output. A fresh artifact run remains.
...
2026-07-29. The code-side conditions are closed and the first schema-v4 artifact
was produced, but V1 rejected it for privacy-scope drift. Next, a Windows
operator runs the corrected published-row filter/privacy derivation, archives
the replacement artifact and checksum, and requests a narrow V1–V3 re-review.
