...
func TestCustomMethodRulesAvoidNestedMethodDoubleCounting(t *testing.T) {
	body := `---------------------------------------------------------------------------------------------------------------------
Total Transaction : 1
----------------------------------------------------------------------------------------------------------------------
...
TXID : 100                                                       DOMAIN (ID) : caller (1)
RESPONSE_TIME : 1000                                             GUID : G1
EXTERNAL_CALL_TIME : 100                                         USER_AGENT : test
APPLICATION : /prod/order/create
...
[0000][10:00:00 000][    0][    0] START
[0001][10:00:00 010][    0][    0] com.acme.RuleEngine.run() [500ms]
[0002][10:00:00 100][    0][    0] com.acme.RuleEngine.validate() [200ms]
[0003][10:00:00 150][    0][    0] SQL-EXECUTE-QUERY [50ms]
[0004][10:00:00 400][    0][    0] EXTERNAL_CALL [HTTP] RULE_ENGINE_CLIENT ( url=/rule/evaluate) [100ms]
[    ][10:00:01 000][    0][    0] END
----------------------------------------------------------------------------------------------------------------------
                TOTAL[1000][   0]
`
	...
	res := Build([]jenniferprofile.FileResult{parsed}, Options{
		CustomAnalysisRules: []models.JenniferCustomAnalysisRule{
			...
			},
			{
				...
				Group:    "internal",
				Source:   "method",
				Patterns: []string{"RuleEngine.validate"},
			},
		},
	})
	...
	if got := stats[0]["total_ms"].(int); got != 350 {
		t.Fatalf("custom stat total_ms = %d, want 350", got)
	}

	groups := res.Series["guid_groups"].([]map[string]any)
	metrics := groups[0]["metrics"].(map[string]any)
	breakdown := metrics["response_time_breakdown"].(map[string]any)
	if got := breakdown["sql_execute_ms"].(int); got != 50 {
		t.Fatalf("sql_execute_ms = %d, want 50", got)
	}
	if got := breakdown["unprofiled_external_call_ms"].(int); got != 100 {
		t.Fatalf("unprofiled_external_call_ms = %d, want 100", got)
	}
	if got := breakdown["method_time_ms"].(int); got != 500 {
		t.Fatalf("method_time_ms = %d, want 500", got)
	}
	...
	if got := slices[0]["value_ms"].(int); got != 350 {
		t.Fatalf("custom value_ms = %d, want 350", got)
	}
}
